...
import eden
from eden.modules.db import FunctionStatus
from .. import SUPERUSER_ID
from eden.sql_db import TestCursor
from eden.tools import (
    config, lazy_classproperty,
    lazy_property, sql, OrderedSet, SQL,
    remove_accents,
)
from eden.tools.func import locked
from eden.tools.lru import LRU
from eden.tools.misc import Collector, format_frame
...
_schema = logging.getLogger('eden.schema')
...
class Registry(Mapping):
    ...
    def new(cls, db_name, force_demo=False, status=None, update_module=False):
        ...
        try:
            ...
            try:
                eden.modules.load_modules(registry, force_demo, status, update_module)
            except Exception:
                eden.modules.reset_modules_state(db_name)
                raise
        except Exception:
    ...
    def init(self, db_name):
        ...
        if config['test_enable'] or config['test_file']:
            from eden.tests.result import EdenTestResult  # noqa: PLC0415
            self._assertion_report = EdenTestResult()
        else:
        ...
        self._db = eden.sql_db.db_connect(db_name, readonly=False)
        ...
        if config['db_replica_host'] is not False or config['test_enable']:  # by default, only use readonly pool if we have a db_replica_host defined. Allows to have an empty replica host for testing
            self._db_readonly = eden.sql_db.db_connect(db_name, readonly=True)
        ...
        with closing(self.cursor()) as cr:
            self.has_unaccent = eden.modules.db.has_unaccent(cr)
            self.has_trigram = eden.modules.db.has_trigram(cr)
    ...
    def init_models(self, cr, model_names, context, install=True):
        ...
        env = eden.api.Environment(cr, SUPERUSER_ID, context)
