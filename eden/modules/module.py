...
import eden
import eden.tools as tools
import eden.release as release
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `eden.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^eden\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("eden.addons.base.maintenance.migrations", "eden.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``eden.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook eden.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in eden.addons.__path__:
        eden.addons.__path__.append(dd)

    # hook eden.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(ad.strip()))
        if ad not in eden.addons.__path__:
            eden.addons.__path__.append(ad)

    # hook eden.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in eden.addons.__path__ and os.path.isdir(base_path):
        eden.addons.__path__.append(base_path)

    # hook eden.upgrade on upgrade-path
    from eden import upgrade
    ...
    # create decrecated module alias from eden.addons.base.maintenance.migrations to eden.upgrade
    spec = importlib.machinery.ModuleSpec("eden.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["eden.addons.base.maintenance"] = maintenance_pkg
    sys.modules["eden.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to eden and "crm"-like to eden.addons
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    if re.search(r"[\/\\]", module):
        return False
    for adp in eden.addons.__path__:
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    sorted_paths = sorted(eden.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name):
    ...
    qualname = f'eden.addons.{module_name}'
...
def get_modules():
    ...
    plist = []
    for ad in eden.addons.__path__:
