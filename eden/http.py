...
r"""\
Eden HTTP layer / WSGI application
...
:class:`~eden.http.Controller`: class and its companion the
:func:`~eden.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an Eden http request. The Eden http request is
...
Eden URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.edencloud.us/documentation/master/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through Eden via py-QWeb form, embed a CSRF
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~eden.service.db.list_dbs`:
    ...
    """
    try:
        dbs = eden.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, Eden will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': eden.service.common.dispatch,
        'db': eden.service.db.dispatch,
        'object': eden.service.model.dispatch,
    }
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~eden.tools.file_path`
        :param filter_ext: See :func:`~eden.tools.file_path`
    ...
    def get_response(
        ...
        **send_file_kwargs
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`eden.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~eden.http.Controller` can use the :func:`~eden.http.route`:
    ...
    :func:`~eden.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(eden.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['eden', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~eden.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['eden', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are Eden addons.
        """
        # Controllers defined outside of eden addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of eden addons can be extended in
...
class Session(collections.abc.MutableMapping):
    ...
    def authenticate(self, dbname, credential):
        ...
        with registry.cursor() as cr:
            env = eden.api.Environment(cr, pre_uid, {})
        ...
        if request and request.session is self and request.db == dbname:
            request.env = eden.api.Environment(request.env.cr, self.uid, self.context)
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~eden.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> eden_ip = socket.gethostbyname('edencloud.us')
        >>> GeoIP(eden_ip).country.iso_code
        'FR'
    """
...
class Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~eden.http.Response`.
        :rtype: Response
...
class Request:
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~eden.addons.base.models.res_users.Users>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~eden.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        try:
            ...
            self.env = eden.api.Environment(cr_readwrite, self.session.uid, self.session.context)
            try:
            ...
            with contextlib.closing(self.registry.cursor(readonly=readonly_cr)) as cr:
                ...
                try:
                    ...
                    if isinstance(exc, HTTPException) and exc.code is None:
                        raise  # bubble up to eden.http.Application.__call__
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~eden.http.Response.load` method for the compatible
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "Eden Session Expired"
...
class Application:
    ...
    def statics(self):
        ...
        """
        mod2path = {}
        for addons_path in eden.addons.__path__:
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint in _generate_routing_rules([''] + eden.conf.server_wide_modules, nodb_only=True):
    ...
    def session_store(self):
        path = eden.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if eden.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
            ...
            try:
                ...
                if self.get_static_file(httprequest.path):
                    ...
                    try:
                        ...
                        if (httprequest.path.startswith('/eden/')
                            or httprequest.path in (
                                '/eden', '/web', '/web/login', '/test_http/ensure_db',
                            )):
