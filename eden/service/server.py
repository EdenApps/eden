...
import eden
from eden.modules import get_modules
from eden.modules.registry import Registry
from eden.release import nt_service_name
from eden.tools import config
from eden.tools.cache import log_ormcache_stats
from eden.tools.misc import stripped_sys_argv, dumpstacks
...
def set_limit_memory_hard():
    ...
    if eden.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'eden.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The EDEN_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("EDEN_MAX_HTTP_THREADS")
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in eden.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="eden.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from eden.addons.base.models.ir_cron import ir_cron
        def _run_cron(cr):
            pg_conn = cr._cnx
            ...
            while config['limit_time_worker_cron'] <= 0 or (time.monotonic() - alive_time) <= config['limit_time_worker_cron']:
                ...
                registries = eden.modules.registry.Registry.registries
        ...
        while True:
            conn = eden.sql_db.db_connect('postgres')
    ...
    def cron_spawn(self):
        ...
        for i in range(eden.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="eden.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from eden.tests.result import _logger as logger  # noqa: PLC0415
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('eden: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(eden.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The EDEN_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("EDEN_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = eden.service.db.list_dbs(True)
        return db_names
    ...
    def start(self):
        ...
        dbconn = eden.sql_db.db_connect('postgres')
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(eden.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            eden.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
The `web` module is provided by the addons found in the `eden` project.
Maybe you forgot to add those addons in your addons_path configuration."""
            _logger.exception('Failed to load server-wide module `%s`.%s', m, msg)

def _reexec(updated_modules=None):
    """reexecute openerp-server process with (nearly) the same arguments"""
    if eden.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    ...
    from eden.tests import loader  # noqa: PLC0415
    from eden.tests.suite import EdenSuite  # noqa: PLC0415
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_enable']:
                from eden.tests import loader  # noqa: PLC0415
                t0 = time.time()
                t0_sql = eden.sql_db.sql_counter
                ...
                if post_install_suite.has_http_case():
                    with registry.cursor() as cr:
                        env = eden.api.Environment(cr, eden.SUPERUSER_ID, {})
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             eden.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the eden http server and cron processor.
    """
    global server
    ...
    if eden.evented:
        server = GeventServer(eden.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(eden.http.root)
    else:
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which Eden is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(eden.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not eden.evented:
        if inotify:
