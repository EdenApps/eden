...
from eden.release import version_info
from eden.sql_db import db_connect
from eden.tools import SQL
from eden.tools.misc import exec_pg_environ, find_pg_tool
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not eden.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and eden.tools.config.verify_admin_password(passwd):
        return True
    raise eden.exceptions.AccessDenied()

# This should be moved to eden.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = eden.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            eden.modules.db.initialize(cr)
            eden.tools.config['load_language'] = lang
            cr.commit()

        registry = eden.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(registry.cursor()) as cr:
            env = eden.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = eden.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = eden.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = eden.tools.config['db_template']
    ...
    try:
        db = eden.sql_db.db_connect(name)
        with db.cursor() as cr:
            cr.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
            if eden.tools.config['unaccent']:
                ...
                # But in the case of Eden, we consider that those data don't
...
def exp_duplicate_database(db_original_name, db_name, neutralize_database=False):
    ...
    eden.sql_db.close_db(db_original_name)
    db = eden.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = eden.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = eden.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)
        if neutralize_database:
            eden.modules.neutralize.neutralize_database(cr)

    from_fs = eden.tools.config.filestore(db_original_name)
    to_fs = eden.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    eden.modules.registry.Registry.delete(db_name)
    eden.sql_db.close_db(db_name)
    ...
    fs = eden.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'eden_dump': '1',
        'db_name': cr.dbname,
        'version': eden.release.version,
        'version_info': eden.release.version_info,
        'major_version': eden.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = eden.tools.config.filestore(db_name)
            ...
            if stream:
                eden.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                eden.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
...
def restore_db(db, dump_file, copy=False, neutralize_database=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        registry = eden.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = eden.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    eden.modules.registry.Registry.delete(old_name)
    eden.sql_db.close_db(old_name)
    ...
    old_fs = eden.tools.config.filestore(old_name)
    new_fs = eden.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    eden.tools.config.set_admin_password(new_password)
    eden.tools.config.save(['admin_passwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        eden.tools.config['update']['base'] = True
        eden.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@eden.tools.mute_logger('eden.sql_db')
...
def list_dbs(force=False):
    if not eden.tools.config['list_db'] and not force:
        raise eden.exceptions.AccessDenied()

    if not eden.tools.config['dbfilter'] and eden.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Eden will not
        ...
        res = sorted(db.strip() for db in eden.tools.config['db_name'].split(','))
        return res
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of Eden
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if eden.tools.sql.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        eden.sql_db.close_db(database_name)
...
def exp_list_lang():
    return eden.tools.misc.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(eden.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
