...
from eden.addons.account.tests.common import AccountTestMockOnlineSyncCommon
...
class BaseTestUi(AccountTestMockOnlineSyncCommon):
    ...
    def main_flow_tour(self):
        ...
        self.start_tour("/eden", 'main_flow_tour', login="admin", timeout=180)
...
class TestUi(BaseTestUi):
    ...
    def test_company_switch_access_error(self):
        ...
        with mute_logger("eden.http"):
            self.start_tour(f"/eden/action-{act_window.id}", "test_company_switch_access_error", login="admin", cookies={"cids": f"{company1.id}-{company2.id}"})
    ...
    def test_company_access_error_redirect(self):
        ...
        with mute_logger("eden.http"):
            self.start_tour(f"/eden/action-{act_window.id}/{record_p2.id}", "test_company_access_error_redirect", login="admin", cookies={"cids": f"{company1.id}"})
    ...
    def test_company_switch_access_error_debug(self):
        ...
        with mute_logger("eden.http"):
            self.start_tour(f"/eden/action-{act_window.id}?debug=assets&cids={current_companies}", "test_company_switch_access_error", login="admin")
