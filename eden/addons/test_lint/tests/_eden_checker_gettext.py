...
def parse_version(s):
    # can't use eden.tools.parse_version because pythonpath is screwed from
    # inside pylint on runbot
    return [s.rjust(3, '0') for s in s.split('.')]


class EdenBaseChecker(BaseChecker):
    ...
    msgs = {
        'E8502': (
            ...
            'gettext-variable',
            'See https://www.edencloud.us/documentation/master/developer/misc/i18n/translations.html#variables'
        ),
        'E8505': (
...
def register(linter):
    linter.register_checker(EdenBaseChecker(linter))
