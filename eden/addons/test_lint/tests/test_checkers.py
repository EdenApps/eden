...
from eden.tools.which import which
from eden.tests.common import TransactionCase

from . import _eden_checker_sql_injection
...
class TestGetTextLint(TestPylintChecks):
    def check(self, testtext):
        return super().check(testtext, "_eden_checker_gettext", "gettext-placeholders")
...
class TestSqlLint(TestPylintChecks):
    def check(self, testtext):
        return super().check(testtext, "_eden_checker_sql_injection", "sql-injection")
    ...
    def test_sql_injection_detection(self):
        ...
        checker = _eden_checker_sql_injection.EdenBaseChecker(self.linter)

        node = _eden_checker_sql_injection.astroid.extract_node("""
        def test(): 
            arg = "test"
            arg = arg + arg
...
class TestI18nChecks(TestPylintChecks):
    def check(self, test_content):
        return super().check(
            test_content, "_eden_checker_gettext", "gettext-variable,gettext-placeholders,gettext-repr"
        )
