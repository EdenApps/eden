...
class TestConflictMarkers(lint_case.LintCase):
    ...
    def test_conflict_markers(self):
        """ Test that there are no conflict markers left in Eden files """

        counter = 0

        eden_path = os.path.abspath(os.path.dirname(eden.__file__))
        paths = [*eden.addons.__path__, eden_path]
        paths.remove(os.path.join(eden_path, 'addons'))  # avoid checking eden/addons twice
