...
import { isIconElement, isSelectionInSelectors, peek } from '@web_editor/js/editor/eden-editor/src/utils/utils';
...
const EdenEditor = EdenEditorLib.EdenEditor;
const getDeepRange = EdenEditorLib.getDeepRange;
const getInSelection = EdenEditorLib.getInSelection;
const isProtected = EdenEditorLib.isProtected;
const rgbToHex = EdenEditorLib.rgbToHex;
const preserveCursor = EdenEditorLib.preserveCursor;
const closestElement = EdenEditorLib.closestElement;
const setSelection = EdenEditorLib.setSelection;
const endPos = EdenEditorLib.endPos;
const hasValidSelection = EdenEditorLib.hasValidSelection;
const parseHTML = EdenEditorLib.parseHTML;
const closestBlock = EdenEditorLib.closestBlock;
const getRangePosition = EdenEditorLib.getRangePosition;
const childNodeIndex = EdenEditorLib.childNodeIndex;
const fillEmpty = EdenEditorLib.fillEmpty;
const isVisible = EdenEditorLib.isVisible;
const getSelectedNodes = EdenEditorLib.getSelectedNodes;
const getDeepestPosition = EdenEditorLib.getDeepestPosition;
const paragraphRelatedElements = EdenEditorLib.paragraphRelatedElements;
...
export class Wysiwyg extends Component {
    ...
    setup() {
        ...
        const getColorPickedHandler = (colorType) => {
            return (params) => {
                if (this.hadNonCollapsedSelectionBeforeColorpicker) {
                    this.edenEditor.historyResetLatestComputedSelection(true);
                }
                // Unstash the mutations now that the color is picked.
                this.edenEditor.historyUnstash();
                ...
                // hides it).
                if (hasValidSelection(this.edenEditor.editable)) {
                    this.edenEditor.deselectTable();
                }
        ...
        const getColorHoverHandler = (colorType) => {
            return (props) => {
                ...
                this.edenEditor.historyPauseSteps();
                try {
                    this._processAndApplyColor(colorType, props.color, true);
                    this.edenEditor._computeHistorySelection();
                } finally {
                    this.edenEditor.historyUnpauseSteps();
                }
            }
        };
        ...
        const colorPaletteCommonOptions = {
            ...
            onColorLeave: () => {
                ...
                this.edenEditor.withoutRollback(() => this.edenEditor.historyRevertCurrentStep());
    ...
    async startEdition() {
        ...
        this.edenEditor = new EdenEditor(this.$editable[0], Object.assign({
            _t: _t,
        ...
        this.edenEditor.addEventListener('contentChanged', function () {
        ...
        if (this.edenEditor.document !== document) {
            this.edenEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.edenEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.edenEditor.document.addEventListener("keyup", this._signalOnline, true);
        }
        ...
        if ($('wrapwrap').length) {
            this.multiSelectionTarget = $().getScrollingTarget(this.edenEditor.document);
            this.multiSelectionTarget.addEventListener('scroll', this.edenEditor.multiselectionRefresh, { passive: true });
        }
        ...
        this._observeEdenFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.edenEditor.observerUnactive();
                const $field = $(this);
                ...
                self.edenEditor.observerActive();
            }
        );
        ...
        this.$editable.on('dblclick', mediaSelector, ev => {
            ...
            if (isEditable) {
                ...
                if (!isProtected(this.edenEditor.document.getSelection().anchorNode)) {
        ...
        if (options.snippets) {
            ...
            $(this.edenEditor.document.body).addClass('editor_enable');
        ...
        if (this.options.getContentEditableAreas) {
            $(this.options.getContentEditableAreas(this.edenEditor)).find('*').off('mousedown mouseup click');
        }
        ...
        $(this.edenEditor.editable).on('mouseup', this._updateEditorUI.bind(this));
        $(this.edenEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.edenEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        ...
        this.edenEditor.document.addEventListener('selectionchange', this._onSelectionChange);
        if (!this.state.showSnippetsMenu) {
            this.setCSSVariables(this.toolbarEl);
        }

        this.edenEditor.addEventListener('preObserverActive', () => {
            ...
            this._onPostSanitize(this.edenEditor.editable);
        });

        if (this.options.autohideToolbar) {
            if (this.edenEditor.isMobile) {
                this.edenEditor.editable.before(this.toolbarEl);
            } else {
    ...
    setupToolbar(toolbarEl) {
        ...
        this.edenEditor.setupToolbar(toolbarEl);
    ...
    destroy() {
        ...
        if (this.edenEditor) {
            this.edenEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.edenEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.edenEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.edenEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            this.edenEditor.destroy();
        }
        ...
        this.multiSelectionTarget?.removeEventListener('scroll', this.edenEditor.multiselectionRefresh, { passive: true });
    ...
    _renderElement() {
        ...
        if (this.options.resizable && !isMobileOS()) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper eden-editor">');
    ...
    getValue(options) {
        ...
        if (this.edenEditor) {
            this.edenEditor.cleanForSave($editable[0]);
    ...
    async saveContent(reload = true, editable = false) {
        ...
        if (!editable) {
            ...
            const editables = "getContentEditableAreas" in this.options ? this.options.getContentEditableAreas(this.edenEditor) : [];
     ...
     */
    setValue(value) {
        this.edenEditor.resetContent(value);
    }
    /**
     * Undo one step of change in the editor.
     */
    undo() {
        this.edenEditor.historyUndo();
    }
    /**
     * Redo one step of change in the editor.
     */
    redo() {
        this.edenEditor.historyRedo();
    }
    /**
     ...
     */
    focus() {
        if (this.edenEditor && !this.edenEditor.historyResetLatestComputedSelection(true)) {
            ...
            const selection = this.edenEditor.document.getSelection();
            selection.removeAllRanges();
            selection.addRange(range);
        }
    }
    getDeepRange() {
        return getDeepRange(this.edenEditor.editable);
    }
    closestElement(...args) {
        return closestElement(...args);
    }
    async cleanForSave(editable = this.edenEditor.editable) {
        if (this.edenEditor) {
            this.edenEditor.cleanForSave(editable);
    ...
    isSelectionInEditable() {
        return this.edenEditor.isSelectionInEditable();
    }
    /**
     * Start or resume the Eden field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeEdenFieldChanges() {
        ...
        if (this.edenFieldObservers) {
            for (let observerData of this.edenFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const edenFieldSelector = '[data-oe-model], [data-oe-translation-source-sha]';
            const $edenFields = this.$editable.find(edenFieldSelector);
            const renderingClassesSelector = this.edenEditor.options.renderingClasses
                .map(className => `.${className}`).join(", ");
            this.edenFieldObservers = [];

            $edenFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.edenEditor.filterMutationRecords(mutations);
                    ...
                    let $nodes = $edenFields.filter(function () {
                        return this !== field;
                    });
                    ...
                    //  `_pauseEdenFieldObservers(field)` call.
                    this.__edenFieldObserversToPause = this.edenFieldObservers.filter(
                    ...
                    for (const node of fieldNodeClone.querySelectorAll(renderingClassesSelector)) {
                        node.classList.remove(...this.edenEditor.options.renderingClasses);
                    }
                    const html = $(fieldNodeClone).html();
                    this.edenEditor.withoutRollback(() => {
                ...
                this.edenFieldObservers.push({field: field, observer: observer});
            });
        }
    }
    /**
     ...
     */
    _pauseEdenFieldObservers() {
        // TODO adapt in master: remove this and directly exclude observers with
        // targets inside the current field (we use `this.edenFieldObservers`
        // as fallback for compatibility here).
        const fieldObserversData = this.__edenFieldObserversToPause || this.edenFieldObservers;
    ...
    openLinkToolsFromSelection() {
        ...
        for (const el of getSelectedNodes(this.edenEditor.editable)) {
            if (
        ...
        if (targetEl.nodeType === Node.ELEMENT_NODE
            ...
            this.edenEditor.dispatchEvent(new Event('activate_image_link_tool'));
            return;
        }
        this.toggleLinkTools();
    }
    ...
    async toggleLinkTools(options = {}) {
        ...
        const linkEl = getInSelection(this.edenEditor.document, 'a');
        ...
        if (this.state.showSnippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.state.linkToolProps) {
                ...
                if (!this.state.linkToolProps || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkToolsInfos.link)) {
                    const { link } = getOrCreateLink({
                        containerNode: this.edenEditor.editable,
                    ...
                    const addHintClasses = () => {
                        this.edenEditor.observerUnactive("hint_classes");
                        link.classList.add('oe_edited_link');
                        $button.addClass('active');
                        this.edenEditor.observerActive("hint_classes");
                    };
                    ...
                    this.state.linkToolProps = {
                        ...
                        wysiwyg: this,
                        editable: this.edenEditor.editable,
                        link,
                ...
                this.edenEditor.document.removeEventListener('click', this._onClick, true);
                ...
                this.edenEditor.document.addEventListener('click', this._onClick, true);
                document.addEventListener('click', this._onClick, true);
            } else {
                this.destroyLinkTools();
            }
        } else {
            const historyStepIndex = this.edenEditor.historySize() - 1;
            this.edenEditor.historyPauseSteps();
            ...
            this.env.services.dialog.add(LinkDialog, {
                ...
                onClose: () => {
                    this.edenEditor.historyUnpauseSteps();
                    this.edenEditor.historyRevertUntil(historyStepIndex)
                }
            });
        }
    }
    ...
    openChatGPTDialog(mode = 'prompt', options={}) {
        const restore = preserveCursor(this.edenEditor.document);
        const params = {
            insert: content => {
                this.edenEditor.historyPauseSteps();
                const insertedNodes = this.edenEditor.execCommand('insert', content);
                this.edenEditor.historyUnpauseSteps();
                ...
                if (start && end) {
                    const divContainer = this.edenEditor.editable.parentElement;
        ...
        if (mode === 'alternatives' || mode === 'translate') {
            params.originalText = this.edenEditor.document.getSelection().toString() || '';
        }
        if (mode === 'translate') {
            params.language = options.language;
        }
        this.edenEditor.document.getSelection().collapseToEnd();
    ...
    removeLink() {
        if (this.state.showSnippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            this.mutex.exec(() => {
                this.edenEditor.dispatchEvent(new Event('deactivate_image_link_tool'));
            });
        } else {
            this.edenEditor.execCommand('unlink');
        }
    }
    /**
    ...
    openMediaDialog(params = {}) {
        const sel = this.edenEditor.document.getSelection();
        ...
        const restoreSelection = preserveCursor(this.edenEditor.document);

        const editable = EdenEditorLib.closestElement(params.node || range.startContainer, '.o_editable') || this.edenEditor.editable;
    ...
    // todo: test me
    showEmojiPicker() {
        const targetEl = this.edenEditor.document.getSelection();
        ...
        this.popover.add(closest, EmojiPicker, {
                onSelect: (str) => {
                    restoreSelection();
                    this.edenEditor.execCommand('insert', str);
                },
            }, {
    ...
    _onMediaDialogSave(params, element) {
        ...
        if (params.node) {
            ...
            this.edenEditor.unbreakableStepUnactive();
            ...
            if (params.node.matches(".oe_unremovable")) {
                ...
                this.edenEditor.historyStep();
                this.edenEditor.observerUnactive("unremovable");
                element.classList.add("oe_unremovable");
                this.edenEditor.observerActive("unremovable");
            } else {
                this.edenEditor.historyStep();
            }

            // Refocus again to save updates when calling `_onWysiwygBlur`
            this.edenEditor.editable.focus();
        } else {
            const result = this.edenEditor.execCommand('insert', element);
            // Refocus again to save updates when calling `_onWysiwygBlur`
            this.edenEditor.editable.focus();
            return result;
        }
    ...
    getInSelection(selector) {
        return getInSelection(this.edenEditor.document, selector);
    }
    /**
    ...
    async getSnippetsMenuClass() {
        const snippetsEditor = await eden.loader.modules.get('@web_editor/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    _configureToolbar(options) {
        ...
        const openTools = e => {
            ...
            switch (e.currentTarget.id) {
                ...
                case 'open-chatgpt': {
                    this.openChatGPTDialog(this.edenEditor.document.getSelection()?.isCollapsed ? 'prompt' : 'alternatives');
                    break;
                }
            }
        };
        ...
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.edenEditor.document});
            const destroyTransfo = () => {
                $image.transfo('destroy');
                $(this.edenEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                this.edenEditor.document.removeEventListener('keydown', keydown);
            }
            const mouseup = () => {
                imgTransformBtn.classList.toggle('active', $image[0].matches('[style*="transform"]'));
            };
            $(this.edenEditor.document).on('mouseup', mouseup);
            ...
            $(this.edenEditor.document).on('mousedown', mousedown);
            ...
            this.edenEditor.document.addEventListener('keydown', keydown);
        });
    ...
    _showImageCrop() {
        ...
        this.edenEditor.toolbarHide();
        $(this.lastMediaClicked).on('image_cropper_destroyed', () => this.edenEditor.toolbarShow());
    }
    /**
     * @private
     * @param {jQuery} $
    ...
    _processAndApplyColor(colorType, color, previewMode) {
        ...
        const selectedTds = this.edenEditor.document.querySelectorAll('td.o_selected_td');
        ...
        let coloredElements = this.edenEditor.execCommand('applyColor', color, colorType === 'text' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        // Some nodes returned by applyColor can be removed of the document by the sanitization in historyStep
        coloredElements = coloredElements.filter(element => this.edenEditor.document.contains(element));
        ...
        if (coloredTds.length) {
            ...
            range.setEnd(...endPos(last));
                const { startContainer, startOffset, endContainer, endOffset } = getDeepRange(this.edenEditor.editable, { range });
            if (isSelForward) {
        ...
        this.edenEditor.updateColorpickerLabels({
     ...
     */
    _updateEditorUI(e) {
        let selection = this.edenEditor.document.getSelection();
        if (!selection) return;
        const anchorNode = selection.anchorNode;
        if (isProtected(anchorNode)) {
            return;
        }
        if (this.edenEditor.document.querySelector(".transfo-container")) {
            return;
        }

        this.edenEditor.automaticStepSkipStack();
        // We need to use the editor's window so the tooltip displays in its
        // document even if it's in an iframe.
        const editorWindow = this.edenEditor.document.defaultView;
        ...
        if (isInMedia) {
            ...
            const selection = this.edenEditor.document.getSelection();
            const range = this.edenEditor.document.createRange();
        ...
        this.edenEditor.updateToolbarPosition();
    ...
    _updateMediaJustifyButton(commandState) {
        ...
        // Change the ID to prevent EdenEditor from controlling it as this is
    ...
    _getBannerCommand(title, emoji, alertClass, iconClass, description, priority) {
        return {
            ...
            isDisabled: () => isSelectionInSelectors('.o_editor_banner') || !this.edenEditor.isSelectionInBlockRoot(),
            callback: () => {
                const bannerElement = parseHTML(this.edenEditor.document, `
                ...
                `).childNodes[0];
                this.edenEditor.execCommand('insert', bannerElement);
                this.edenEditor.activateContenteditable();
    ...
    _getPowerboxOptions() {
        ...
        const commands = [
            ...
            this._getBannerCommand(_t('Banner Danger'), '❌', 'danger', 'fa-exclamation-circle', _t('Insert a danger banner'), 21),
            {
                ...
                isDisabled: () => !this.edenEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.edenEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                category: _t('Structure'),
                ...
                isDisabled: () => !this.edenEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.edenEditor.execCommand('setTag', 'pre');
                },
            },
            {
                ...
                callback: async () => {
                    ...
                    if (currentUser && currentUser.signature) {
                        this.edenEditor.execCommand('insert', parseHTML(this.edenEditor.document, currentUser.signature));
                    }
                },
            },
            {
            ...
            commands.push(
                {
                    ...
                    callback: () => this.edenEditor.execCommand('columnize', 2, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.edenEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                        const anchor = this.edenEditor.document.getSelection().anchorNode;
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.edenEditor.execCommand('columnize', 3, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.edenEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.edenEditor.execCommand('columnize', 4, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.edenEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.edenEditor.execCommand('columnize', 0),
                    isDisabled: () => {
                        if (!this.edenEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
    ...
    async _saveElement($el, context) {
        ...
        // remove ZeroWidthSpace from eden field value
        // ZeroWidthSpace may be present from EdenEditor edition process
    ...
    _onSelectionChange() {
        if (this.linkPopover && this.isSelectionInEditable()) {
            const selectionInLink = getInSelection(this.edenEditor.document, 'a') === this.linkPopover.target;
            const isVisible = this.linkPopover.el.offsetParent;
            if (
                isVisible &&
                (
                    (this.options.autohideToolbar && !this.edenEditor.document.getSelection().isCollapsed) ||
                    !selectionInLink
                )
            ) {
                this.linkPopover.hide();
    ...
    async _signalOnline() {
        ...
        this.ptp.notifyAllClients('oe_history_step', peek(this.edenEditor.historyGetSteps()), { transport: 'rtc' });
    }
    /**
    ...
    async _processMissingSteps(missingSteps) {
        ...
        this.ptp && this.edenEditor.onExternalHistorySteps(missingSteps);
        return true;
    }
    _showConflictDialog() {
        if (this._conflictDialogOpened) return;
        const content = markup(this.edenEditor.editable.cloneNode(true).outerHTML);
    ...
    _getNewPtp() {
        ...
        return new PeerToPeer({
            ...
            onRequest: {
                ...
                get_missing_steps: (params) => this.edenEditor.historyGetMissingSteps(params.requestPayload),
                get_history_from_snapshot: () => this._getHistorySnapshot(),
                get_collaborative_selection: () => this.edenEditor.getCurrentCollaborativeSelection(),
                recover_document: (params) => {
                    const { serverDocumentId, fromStepId } = params.requestPayload;
                    if (!this.edenEditor.historyGetBranchIds().includes(serverDocumentId)) {
                        return;
                    }
                    return {
                        missingSteps: this.edenEditor.historyGetMissingSteps({ fromStepId }),
            ...
            onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                switch (notificationName) {
                    case 'ptp_remove':
                        this.edenEditor.multiselectionRemove(notificationPayload);
                        break;
                    case 'ptp_disconnect':
                        this.ptp.removeClient(fromClientId);
                        this.edenEditor.multiselectionRemove(fromClientId);
                        break;
                    case 'rtc_data_channel_open': {
                        ...
                        const getClientNamePromise = this.requestClient(
                            ...
                            this.edenEditor.multiselectionRefresh();
                        });
                    ...
                    case 'oe_history_step':
                        if (this._historySyncFinished) {
                            this.edenEditor.onExternalHistorySteps([notificationPayload]);
                        } else {
                    ...
                    case 'oe_history_set_selection': {
                        ...
                        this.edenEditor.onExternalMultiselectionUpdate(selection);
                        break;
                    }
                }
            }
    ...
    async _setCollaborativeSelection(fromClientId) {
        ...
        if (remoteSelection) {
            this.edenEditor.onExternalMultiselectionUpdate(remoteSelection);
        }
    }
    /**
    ...
    _isLastDocumentStale() {
        ...
        return !this.edenEditor.historyGetBranchIds().includes(this._serverLastStepId);
    }
    /**
            ...
            for (const peerId of allPeers) {
                this.requestClient(
                    ...
                    'recover_document', {
                        ...
                        fromStepId: peek(this.edenEditor.historyGetBranchIds()),
                    },
                    { transport: 'rtc' }
                ).then((response) => {
                    nbPendingResponses--;
    ...
    async _resetFromServerAndResyncWithClients() {
        ...
        // lastHistoryId will be different if the eden bus did not have time to
        // notify the user.
    ...
    async _resetFromClient(fromClientId, resetCollabCount) {
        ...
        if (this._historyStepsBuffer.length) {
            this.edenEditor.onExternalHistorySteps(this._historyStepsBuffer);
            this._historyStepsBuffer = [];
        }
    ...
    async resetValue(value) {
        ...
        if (this._serverLastStepId) {
            this.edenEditor.historySetInitialId(this._serverLastStepId);
        }
    }
    /**
    ...
    async resetEditor(value, options) {
        ...
        this._currentClientId = this._generateClientId();
        this.edenEditor.collaborationSetClientId(this._currentClientId);
        this.resetValue(value);
    ...
    _getHistorySnapshot() {
        return Object.assign(
            {},
            this.edenEditor.historyGetSnapshotSteps(),
    ...
    _applySnapshot(snapshot) {
        ...
        this._historySyncAtLeastOnce = true;
        this.edenEditor.historyResetFromSteps(steps, historyIds);
        this.edenEditor.historyResetLatestComputedSelection();
        return true;
    }
    /**
    ...
    _onPostSanitize(node) {
        ...
        // solution is merged.
        if (node?.querySelectorAll && this.edenEditor && !this.edenEditor._fixLinkMutatedElements) {
    ...
    _attachHistoryIds(editable = this.edenEditor.editable) {
        if (this.options.collaborative) {
            ...
            const historyIds = this.edenEditor.historyGetBranchIds().join(',');
    ...
    _beforeAnyCommand() {
        ...
        for (const node of EdenEditorLib.getTraversedNodes(this.$editable[0])) {
