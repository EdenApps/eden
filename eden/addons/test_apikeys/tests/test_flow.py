...
from eden.tests import tagged, get_db_name, HttpCase
from eden.addons.auth_totp.tests.test_totp import TestTOTPMixin
...
class TestAPIKeys(TestTOTPMixin, HttpCase):
    ...
    def test_addremove(self):
        db = get_db_name()
        self.start_tour('/eden', 'apikeys_tour_setup', login=self.user_test.login)
        ...
        self.start_tour('/eden', 'apikeys_tour_teardown', login=self.user_test.login)

    def test_apikeys_totp(self):
        db = get_db_name()
        self.install_totphook()
        self.start_tour('/eden', 'apikeys_tour_setup', login=self.user_test.login)
        self.start_tour('/eden', 'totp_tour_setup', login=self.user_test.login)
