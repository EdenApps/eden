...
@eden.tests.tagged('post_install', '-at_install', 'post_install_l10n')
class TestReports(eden.tests.TransactionCase):
...
class TestReportsRenderingCommon(eden.tests.HttpCase):
    ...
    def save_pdf(self):
        ...
        eden.tests.save_test_file(self._testMethodName, self.last_pdf_content, 'pdf_', 'pdf', document_type='Report PDF', logger=_logger)
...
@eden.tests.tagged('post_install', '-at_install', 'pdf_rendering')
...
@eden.tests.tagged('post_install', '-at_install', '-standard', 'pdf_rendering')
...
class TestAggregatePdfReports(eden.tests.HttpCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
