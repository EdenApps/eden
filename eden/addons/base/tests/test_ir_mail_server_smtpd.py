...
from eden import modules
from eden.exceptions import UserError
from eden.tools import config, file_path, mute_logger
...
@patch('eden.addons.base.models.ir_mail_server.SMTP_TIMEOUT', .1)
...
class TestIrMailServerSMTPD(TransactionCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        # decrease aiosmtpd verbosity, eden INFO = aiosmtpd WARNING
        ...
        # force aiosmtpd/eden to bind/connect to a fixed ipv4 OR ipv6
        # address.
    ...
    def test_man_in_the_middle_matrix(self):
        ...
        traffic in between the Eden server and the legitimate SMTP
        server.
        """
        mail_server = self.env['ir.mail_server'].create({
            'name': 'test smtpd',
