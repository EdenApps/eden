...
from eden.tools.translate import quote, unquote, xml_translate, html_translate, TranslationImporter, TranslationModuleReader
from eden.tests.common import TransactionCase, BaseCase, new_test_user, tagged

_stats_logger = logging.getLogger('eden.tests.stats')
    ...
    def test_translate_xml_a(self):
        ...
        source = """<t t-name="stuff">
                        <ul class="nav navbar-nav">
                            <li class="nav-item">
                                <a class="nav-link oe_menu_leaf" href="/eden/action-54?menu_id=42">
...
class TestLanguageInstall(TransactionCase):
    def test_language_install(self):
        ...
        with patch('eden.addons.base.models.ir_module.Module._load_module_terms', _load_module_terms):
            wizard.lang_install()
...
class TestTranslationExport(TransactionCase):
    ...
    def test_export_translatable_resources(self):
        ...
        with self.assertNoLogs('eden.tools.translate', "ERROR"):
...
class TestTranslation(TransactionCase):
    ...
    #     country_1 = Country.create({'name': 'Eden'})
    #     country_1.with_context(lang='fr_FR').name = 'Eden_Fr'
    #     country_1.flush_recordset()
    #
    #     country_2 = Country.create({'name': 'Eden2'})
    #     with self.assertRaises(IntegrityError), mute_logger('eden.sql_db'):
    #         country_2.name = 'Eden'
    #         country_2.flush_recordset()
    #
    #     with self.assertRaises(IntegrityError), mute_logger('eden.sql_db'):
    #         country_3 = Country.create({'name': 'Eden'})
