...
from eden.addons.base.tests import test_mail_examples
from eden.addons.base.tests.common import MockSmtplibCase
...
class TestIrMailServer(TransactionCase, MockSmtplibCase):
    ...
    def test_mail_server_priorities(self):
        ...
        simple Eden DB so we have to spoof the FROM otherwise we cannot send
        any email. """
    ...
    @mute_logger('eden.models.unlink', 'eden.addons.base.models.ir_mail_server')
    ...
    def test_mail_server_config_bin(self):
        """ Test the configuration provided in the eden-bin arguments. This config
        ...
        eden-bin argument "--from-filter".
        """
        IrMailServer = self.env['ir.mail_server']

        # Remove all mail server so we will use the eden-bin arguments
        ...
        # for from_filter in ICP, overwrite the one from eden-bin
    ...
    def test_mail_server_config_cli(self):
        ...
        "cli". It should take the configuration from the eden-bin argument. The
        ...
        for mail_from, (expected_smtp_from, expected_msg_from, expected_mail_server) in zip(
            [
                # check that the CLI server take the configuration in the eden-bin argument
