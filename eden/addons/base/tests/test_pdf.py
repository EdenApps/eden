...
from eden.tools import pdf
from eden.tools.misc import file_open
from eden.tools.pdf import reshape_text
import io
...
class TestPdf(TransactionCase):
    ...
    def setUp(self):
        ...
        self.minimal_pdf_reader = pdf.EdenPdfFileReader(self.minimal_reader_buffer)

    def test_eden_pdf_file_reader(self):
        ...
        r = pdf.EdenPdfFileReader(io.BytesIO(out.getvalue()))
        self.assertEqual(len(list(r.getAttachments())), 1)

    def test_eden_pdf_file_writer(self):
        attachments = list(self.minimal_pdf_reader.getAttachments())
        self.assertEqual(len(attachments), 0)
        ...
        for count, (name, data) in enumerate([
            ...
            pdf_writer = pdf.EdenPdfFileWriter()
    ...
    def test_eden_pdf_file_reader_with_owner_encryption(self):
        pdf_writer = pdf.EdenPdfFileWriter()
        pdf_writer.cloneReaderDocumentRoot(self.minimal_pdf_reader)
        ...
        with io.BytesIO(encrypted_content) as reader_buffer:
            pdf_reader = pdf.EdenPdfFileReader(reader_buffer)
    ...
    def test_merge_pdf(self):
        ...
        merged_pdf_reader = pdf.EdenPdfFileReader(merged_reader_buffer)
    ...
    def test_branded_file_writer(self):
        ...
        self.assertEqual(pdf_info['/Producer'], 'Eden')
        self.assertEqual(pdf_info['/Creator'], 'Eden')
        reader_buffer.close()
