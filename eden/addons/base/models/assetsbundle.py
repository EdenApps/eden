...
from eden import release, SUPERUSER_ID, _
from eden.http import request
from eden.tools import (func, misc, transpile_javascript,
    is_eden_module, SourceMapGenerator, profiler, OrderedSet)
from eden.tools.json import scriptsafe as json
from eden.tools.constants import SCRIPT_EXTENSIONS, STYLE_EXTENSIONS
from eden.tools.misc import file_open, file_path
            ...
            if self.templates:
                ...
                template_bundle = textwrap.dedent(f"""
                    ...
                    *******************************************/
                    ...
                    eden.define("{self.name}.bundle.xml", ["@web/core/templates"], function(require) {{
                        "use strict";
        ...
        * 't-inherit-mode':  'primary' or 'extension'.
        ...
        for asset in self.templates:
            ...
            try:
                ...
                template = content if content.startswith('<eden>') else f'<templates>{asset.content}</templates>'
    ...
    def compile_css(self, compiler, source):
        ...
        def sanitize(matchobj):
            ...
            if '.' not in ref and line not in imports and not ref.startswith(('.', '/', '~')):
                imports.append(line)
                return line
            msg = "Local import '%s' is forbidden for security reasons. Please remove all @import {your_file} imports in your custom files. In Eden you have to import all files in the assets, and not through the @import statement." % ref
            _logger.warning(msg)
...
class JavascriptAsset(WebAsset):
    ...
    def is_transpiled(self):
        if self._is_transpiled is None:
            self._is_transpiled = bool(is_eden_module(self.url, super().content))
