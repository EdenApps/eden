...
BASE_VERSION = eden.modules.get_manifest('base')['version']
...
EDEN_NOTIFY_FUNCTION = os.getenv('EDEN_NOTIFY_FUNCTION', 'pg_notify')
...
class ir_cron(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['usage'] = 'ir_cron'
        if os.getenv('EDEN_NOTIFY_CRON_CHANGES'):
    ...
    def _check_modules_state(cls, cr, jobs):
        ...
        # reset_module_states.
        eden.modules.reset_modules_state(cr.dbname)
    ...
    def write(self, vals):
        self._try_lock()
        if ('nextcall' in vals or vals.get('active')) and os.getenv('EDEN_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
    ...
    def _trigger_list(self, at_list):
        ...
        if min(at_list) <= now or os.getenv('EDEN_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
        return triggers

    def _notifydb(self):
        """ Wake up the cron workers
        The EDEN_NOTIFY_CRON_CHANGES environment variable allows to force the notifydb on both
        ir_cron modification and on trigger creation (regardless of call_at)
        """
        with eden.sql_db.db_connect('postgres').cursor() as cr:
            cr.execute(SQL("SELECT %s('cron_trigger', %s)", SQL.identifier(EDEN_NOTIFY_FUNCTION), self.env.cr.dbname))
