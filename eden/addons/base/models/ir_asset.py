...
import eden
import eden.modules.module  # get_manifest, don't from-import it
from eden import api, fields, models, tools
from eden.tools import misc
from eden.tools.constants import ASSET_EXTENSIONS, EXTERNAL_ASSET
...
class IrAsset(models.Model):
    ...
    def _fill_asset_paths(self, bundle, asset_paths, seen, addons, installed, **assets_params):
        ...
        for addon in addons:
            for command in eden.modules.module._get_manifest_cached(addon)['assets'].get(bundle, ()):
    ...
    def _topological_sort(self, addons_tuple):
        ...
        def mapper(addon):
            manif = eden.modules.module._get_manifest_cached(addon)
    ...
    def _get_installed_addons_list(self):
        ...
        return self.env.registry._init_modules.union(eden.conf.server_wide_modules or [])
    ...
    def _get_paths(self, path_def, installed):
        ...
        Ex: ('/base/static/file.js', '/home/user/source/eden/eden/addons/base/static/file.js', 643636800)
        ...
        addon = path_parts[0]
        addon_manifest = eden.modules.module._get_manifest_cached(addon)
