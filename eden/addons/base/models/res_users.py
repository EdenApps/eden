...
from eden import api, fields, models, tools, SUPERUSER_ID, _, Command
from eden.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from eden.exceptions import AccessDenied, AccessError, UserError, ValidationError
from eden.http import request, DEFAULT_LANG
from eden.osv import expression
from eden.tools import is_html_empty, partition, frozendict, lazy_property, SQL, SetDefinitions
...
class Users(models.Model):
    ...
    def _unlink_except_master_data(self):
        ...
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Eden (updates, module installation, ...)'))
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: eden.exceptions.AccessDenied when old password is wrong
        :raise: eden.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this Eden portal anymore.
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your Eden is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "Eden in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.edencloud.us/documentation/master/administration/install/deploy.html#https for details.",
                    source
                )
...
class GroupsImplied(models.Model):
    ...
    def _get_group_definitions(self):
        """ Return the definition of all the groups as a :class:`~eden.tools.SetDefinitions`. """
