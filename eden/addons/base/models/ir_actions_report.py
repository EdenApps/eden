...
from eden import api, fields, models, tools, SUPERUSER_ID, _
from eden.exceptions import UserError, AccessError, RedirectWarning
from eden.service import security
from eden.tools.safe_eval import safe_eval, time
from eden.tools.misc import find_in_path
from eden.tools import check_barcode_encoding, config, is_html_empty, parse_version, split_every
from eden.http import request, root
from eden.tools.pdf import PdfFileWriter, PdfFileReader, PdfReadError
from eden.osv.expression import NEGATIVE_TERM_OPERATORS, FALSE_DOMAIN
...
# here to init the T1 fonts cache at the start-up of Eden so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at Eden start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start Eden with at least two workers to print a pdf version of the reports.')
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start Eden with at least two workers to convert images to html.')
    else:
...
class IrActionsReport(models.Model):
    ...
    def _handle_merge_pdfs_error(self, error=None, error_stream=None):
        raise UserError(_("Eden is unable to merge the generated PDFs."))
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        ...
        if error_record_ids:
            ...
            raise RedirectWarning(
                message=_('Eden is unable to merge the generated PDFs because of %(num_errors)s corrupted file(s)', num_errors=num_errors),
                action=action,
