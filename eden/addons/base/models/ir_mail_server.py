...
from eden import api, fields, models, tools, _, modules
from eden.exceptions import UserError
from eden.tools import formataddr, email_normalize, encapsulate_email, email_domain_extract, email_domain_normalize, human_size

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('eden.tests')
...
class IrMailServer(models.Model):
    ...
    from_filter = fields.Char(
        ...
        help='Comma-separated list of addresses or domains for which this server can be used.\n'
             'e.g.: "notification@edencloud.us" or "edencloud.us"')
    ...
    def connect(self, host=None, port=None, user=None, password=None, encryption=None,
        ...
        """Returns a new SMTP connection to the given SMTP server.
           ...
           :param ssl_certificate: filename of the SSL certificate used for authentication
               Used when no mail server is given and overwrite  the eden-bin argument "smtp_ssl_certificate"
           :param ssl_private_key: filename of the SSL private key used for authentication
               Used when no mail server is given and overwrite  the eden-bin argument "smtp_ssl_private_key"
        ...
        if smtp_encryption == 'ssl':
            if 'SMTP_SSL' not in smtplib.__all__:
                raise UserError(
                    _("Your Eden Server does not support SMTP-over-SSL. "
        ...
        # Store the "from_filter" of the mail server / eden-bin argument to  know if we
    ...
    def _find_mail_server(self, email_from, mail_servers=None):
        ...
        - Mail server to use to send the email (None if we use the eden-bin arguments)
        ...
        # Skip if passed email_from is False (example Edenbot has no email address)
        ...
        # 5: SMTP config in eden-bin arguments
        from_filter = self.env['ir.mail_server']._get_default_from_filter()
