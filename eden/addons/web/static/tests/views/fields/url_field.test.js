...
test("UrlField in form view", async () => {
    ...
    await fieldInput("url").edit("https://www.edencloud.us");
    expect(`.o_field_widget input[type="text"]`).toHaveValue("https://www.edencloud.us");
});
...
test("onchange scenario", async () => {
    ...
    Product._records = [{ id: 1, url: "edencloud.us", url_source: "another.com" }];
    await mountView({
        type: "form",
        resModel: "product",
        resId: 1,
    ...
    expect(".o_field_widget[name=url]").toHaveText("eden.com");
...
test("with non falsy, but non url value", async () => {
    Product._fields.url.default = "eden://hello";
    await mountView({
        type: "form",
        resModel: "product",
        arch: `<form><field name="url" widget="url"/></form>`,
    });
    expect(".o_field_widget[name=url] a").toHaveAttribute("href", "http://eden://hello");
});
