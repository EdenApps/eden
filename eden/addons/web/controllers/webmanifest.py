...
from eden import http, modules
from eden.exceptions import AccessError
from eden.http import request
from eden.tools import file_open, file_path, image_process
...
class WebManifest(http.Controller):
        ...
        for module in module_ids:
            ...
            if data:
                shortcuts.append({
                    ...
                    'url': '/eden?menu_id=%s' % data.mapped('res_id')[0],
    ...
    def _get_webmanifest(self):
        web_app_name = request.env['ir.config_parameter'].sudo().get_param('web.web_app_name', 'Eden')
        manifest = {
            'name': web_app_name,
            'scope': '/eden',
            'start_url': '/eden',
            'display': 'standalone',
            'background_color': '#1C9300',
            'theme_color': '#1C9300',
            'prefer_related_applications': False,
        }
        icon_sizes = ['192x192', '512x512']
        manifest['icons'] = [{
            'src': '/web/static/img/eden-icon-%s.png' % size,
            'sizes': size,
            'type': 'image/png',
    ...
    def service_worker(self):
        response = request.make_response(
            ...
            [
                ...
                ('Service-Worker-Allowed', '/eden'),
            ]
        )
        return response

    def _get_service_worker_content(self):
        """ Returns a ServiceWorker javascript file scoped for the backend (aka. '/eden')
        """
    ...
    def _icon_path(self):
        return 'web/static/img/eden-icon-192x192.png'

    @http.route('/eden/offline', type='http', auth='public', methods=['GET'], readonly=True)
    def offline(self):
        """ Returns the offline page delivered by the service worker """
        return request.render('web.webclient_offline', {
            'eden_icon': base64.b64encode(file_open(self._icon_path(), 'rb').read())
        })
    ...
    def scoped_app(self, app_id, path='', app_name=''):
        ...
        scoped_app_values = {
            'app_id': app_id,
            'apple_touch_icon': '/web/static/img/eden-icon-ios.png',
            'app_name': app_name,
            'path': path,
    ...
    def scoped_app_manifest(self, app_id, path, app_name=''):
        """ Returns a WebManifest dedicated to the scope of the given app. A custom scope and start
            url are set to make sure no other installed PWA can overlap the scope (e.g. /eden)
        """
        path = unquote(path)
        app_name = unquote(app_name) if app_name else self._get_scoped_app_name(app_id)
        webmanifest = {
