...
@eden.tests.tagged('click_all', 'post_install', '-at_install', '-standard')
class TestMenusAdmin(eden.tests.HttpCase):
    ...
    def test_01_click_everywhere_as_admin(self):
        ...
        for app_id in menus['root']['children']:
            with self.subTest(app=menus[app_id]['name']):
                ...
                self.browser_js("/eden", "eden.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere('%s');" % menus[app_id]['xmlid'], "eden.isReady === true", login="admin", timeout=1200, success_signal="clickbot test succeeded")
...
class TestMenusDemo(HttpCaseWithUserDemo):
    ...
    def test_01_click_everywhere_as_demo(self):
        ...
        for app_id in menus['root']['children']:
            with self.subTest(app=menus[app_id]['name']):
                _logger.runbot('Testing %s', menus[app_id]['name'])
                self.browser_js("/eden", "eden.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere('%s');" % menus[app_id]['xmlid'], "eden.isReady === true", login="demo", timeout=1200, success_signal="clickbot test succeeded")

@eden.tests.tagged('post_install', '-at_install')
class TestMenusAdminLight(eden.tests.HttpCase):
    allow_end_on_form = True

    @classmethod
    def _request_handler(cls, s: Session, r: PreparedRequest, /, **kw):
        # mock edenfin requests
    ...
    def test_01_click_apps_menus_as_admin(self):
        ...
        self.browser_js("/eden", "eden.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere(undefined, true);", "eden.isReady === true", login="admin", timeout=120, success_signal="clickbot test succeeded")
...
class TestMenusDemoLight(HttpCaseWithUserDemo):
    ...
    def test_01_click_apps_menus_as_demo(self):
        ...
        self.browser_js("/eden", "eden.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere(undefined, true);", "eden.isReady === true", login="demo", timeout=120, success_signal="clickbot test succeeded")
