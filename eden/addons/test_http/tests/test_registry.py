...
from eden.sql_db import close_db, db_connect
from eden.tests import HOST, BaseCase, Like, get_db_name, tagged
from eden.tools import lazy_property, mute_logger, SQL
...
class TestHttpRegistry(BaseCase):
    ...
    def setUpClass(cls):
        ...
        cls.addClassCleanup(lazy_property.reset_all, eden.http.root)
        cls.classPatch(eden.conf, 'server_wide_modules', ['base', 'web', 'test_http'])

        # make sure there are always many databases, to break monodb
        cls._db_list = cls.startClassPatcher(patch('eden.http.db_list'))
        cls._db_list.return_value = ['postgres', get_db_name()]
        cls.startClassPatcher(patch('eden.http.db_filter',
    ...
    def authenticate(self, *, db=None):
        session = eden.http.root.session_store.new()
        session.update(eden.http.get_default_session(), db=db or get_db_name())
    ...
    def url_open(self, path, *, allow_redirects=False):
        ...
        url = urljoin(f"http://{HOST}:{eden.tools.config['http_port']}", path)
    ...
    def test_signaling(self):
        ...
        with self.assertLogs('eden.modules.registry', logging.INFO) as capture:
            res = self.url_open('/test_http/ensure_db')
            self.assertEqual(res.status_code, 200)
        self.assertEqual(capture.output, [
            "INFO:eden.modules.registry:Reloading the model registry after database signaling.",
            Like("INFO:eden.modules.registry:Registry loaded in ...s"),
        ])
    ...
    def test_missing_db(self):
        ...
        with self.assertLogs('eden.http', logging.WARNING) as capture:
            res = self.url_open('/test_http/ensure_db')
        ...
        self.assertEqual(capture.output, [
            Like("WARNING:eden.http:Database or registry unusable, trying without\n"
    ...
    def test_corrupt_ir_module_module_table(self):
        ...
        with self.assertLogs('eden.modules.registry', logging.ERROR) as capture1, \
             self.assertLogs('eden.http', logging.WARNING) as capture2:
            res = self.url_open('/test_http/greeting-public')
            self.assertEqual(res.status_code, 404)
        self.assertEqual(capture1.output, [
            "ERROR:eden.modules.registry:Failed to load registry",
        ])
