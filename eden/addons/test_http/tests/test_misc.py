...
import eden
from eden.http import root, content_disposition
from eden.tests import tagged
from eden.tests.common import HOST, new_test_user, get_db_name, BaseCase
from eden.tools import config, file_path, parse_version
from eden.addons.test_http.controllers import CT_JSON

from eden.addons.test_http.utils import TEST_IP
...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc2_local_redirect(self):
        def local_redirect(path):
            fake_req = eden.tools.misc.DotDict(db=False)
            return eden.http.Request.redirect(fake_req, path, local=True).headers['Location']
    ...
    def test_misc5_geoip(self):
        ...
        with patch.dict(eden.tools.config.options, {'proxy_mode': True}):
    ...
    def test_misc6_upload_file_retry(self):
        from eden.addons.test_http import controllers  # pylint: disable=C0415
...
class TestHttpEnsureDb(TestHttpBase):
    ...
    def test_ensure_db1_grant_db(self):
        ...
        self.assertEqual(eden.http.root.session_store.get(res.cookies['session_id']).db, 'db0')
    ...
    def test_ensure_db3_change_db(self):
        ...
        new_session = eden.http.root.session_store.get(res.cookies['session_id'])
    ...
    def test_ensure_db4_unicode(self):
        ...
        self.assertEqual(
            eden.http.root.session_store.get(res.cookies['session_id']).db,
            'basededonnée1')
