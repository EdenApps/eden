...
from eden.http import SESSION_LIFETIME
from eden.tools import config, lazy_property, mute_logger
from eden.tests import get_db_name, tagged
...
class TestHttpSession(TestHttpBase):
    ...
    @mute_logger('eden.http')  # greeting_none called ignoring args {'debug'}
    ...
    def test_session1_default_session(self):
        ...
        with patch.object(eden.http.root.session_store, 'save') as mock_save:
    ...
    def test_session3_logout_15_0_geoip(self):
        ...
        with self.assertLogs('eden.http', level='WARNING') as (_, warnings):
            res = self.multidb_url_open('/test_http/ensure_db', dblist=['db1', 'db2'])

        self.assertEqual(warnings, [
            "WARNING:eden.http:Logged into database 'idontexist', but dbfilter rejects it; logging session out.",
        ])
    ...
    @patch("eden.http.root.session_store.vacuum")
    def test_session8_gc_ignored_no_db_name(self, mock):
        with patch.dict(os.environ, {'EDEN_SKIP_GC_SESSIONS': ''}):
            self.env['ir.http']._gc_sessions()
            mock.assert_called_once()

        with patch.dict(os.environ, {'EDEN_SKIP_GC_SESSIONS': '1'}):
            mock.reset_mock()
            self.env['ir.http']._gc_sessions()
    ...
    def test_session9_logout(self):
        ...
        self.assertTrue(eden.http.root.session_store.get(sid), "session should exist")
        self.url_open('/web/session/logout', allow_redirects=False).raise_for_status()
        self.assertFalse(eden.http.root.session_store.get(sid), "session should not exist")
    ...
    def test_session10_explicit_session(self):
        ...
        with self.assertLogs('eden.http') as capture:
        ...
        self.assertRegex(capture.output[0],
            r"^WARNING:eden.http:<function eden\.addons\.\w+\.controllers\.\w+\.logout> "
...
class TestSessionStore(HttpCaseWithUserDemo):
    def setUp(self):
        ...
        lazy_property.reset_all(eden.http.root)
        self.addCleanup(lazy_property.reset_all, eden.http.root)
    ...
    def test01_session_nan(self):
        ...
        with self.assertLogs('eden.http', level='WARNING') as logs:
            self.assertEqual(eden.http.get_session_max_inactivity(self.env), SESSION_LIFETIME)
            self.assertEqual(logs.output[0], "WARNING:eden.http:Invalid value for 'sessions.max_inactivity_seconds', using default value.")
    ...
    def test02_session_lifetime_1week(self):
        ...
        with freeze_time() as freeze:
            ...
            self.env['ir.http']._gc_sessions()
            session_from_store = eden.http.root.session_store.get(session.sid)
    ...
    def test04_session_lifetime_nodb(self):
        ...
        with freeze_time() as freeze:
            ...
            self.env['ir.http']._gc_sessions()
            session_from_store = eden.http.root.session_store.get(session)
