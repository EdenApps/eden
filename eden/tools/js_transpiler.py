"""
This code is what let us use ES6-style modules in eden.
Classic Eden modules are composed of a top-level :samp:`eden.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@eden-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** eden-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual eden one with minimal changes.
...
def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom eden modules.
    ...
    legacy_eden_define = get_aliased_eden_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_eden_define, module_path, dependencies),
    ]
    for s in steps:
        content = s(content)
    if legacy_eden_define:
        content += legacy_eden_define
    return content
...
def url_to_module_path(url):
    """
    Eden modules each have a name. (eden.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_eden_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the eden.define call.
    ...
    """
    return f"""eden.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
EDEN_MODULE_RE = re.compile(r"""
    \s*                             # starting white space
    \/(\*|\/)                       # /* or //
    .*                              # any comment in between (optional)
    @eden-module                    # '@eden-module' statement
...
def is_eden_module(url, content):
    """
    Detect if the file is a native eden module.
    We look for a comment containing @eden-module.

    :param url:
    :param content: source code
    :return: is this a eden module that need transpilation ?
    """
    result = EDEN_MODULE_RE.match(content)
...
def get_aliased_eden_define_content(module_path, content):
    """
    ...
    > eden.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @eden-module */
        // after
        /** @eden-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @eden-module */
        // after
        /** @eden-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = EDEN_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\neden.define(`%s`, ['%s'], function (require) {
                        return require('%s');
