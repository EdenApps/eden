...
"""
Miscellaneous tools used by Eden.
"""
...
from eden.loglevels import exception_to_unicode, get_encodings, ustr  # noqa: F401
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if eden.tools.config['db_host']:
        env['PGHOST'] = eden.tools.config['db_host']
    if eden.tools.config['db_port']:
        env['PGPORT'] = str(eden.tools.config['db_port'])
    if eden.tools.config['db_user']:
        env['PGUSER'] = eden.tools.config['db_user']
    if eden.tools.config['db_password']:
        env['PGPASSWORD'] = eden.tools.config['db_password']
    return env
...
def file_path(file_path: str, filter_ext: tuple[str, ...] = ('',), env: Environment | None = None) -> str:
    ...
    addons_paths = [*eden.addons.__path__, root_path, *temporary_paths]
...
def file_open(name: str, mode: str = "r", filter_ext: tuple[str, ...] = (), env: Environment | None = None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/eden/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def file_open_temporary_directory(env: Environment):
    ...
    Examples::
        ...
        >>> with eden.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with eden.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('eden.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('eden.foo.bar'):
            do_suff()
    """
...
def format_date(
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = eden.fields.Datetime.from_string(value)
            value = eden.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = eden.fields.Datetime.from_string(value)
...
def format_datetime(
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = eden.fields.Datetime.from_string(value)
    else:
        timestamp = value
...
def limited_field_access_token(record, field_name, timestamp=None):
    ...
    :type record: class:`eden.models.Model`
