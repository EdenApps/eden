...
PYTHON_TRANSLATION_COMMENT = 'eden-python'

# translation used for javascript code in web client
JAVASCRIPT_TRANSLATION_COMMENT = 'eden-javascript'
...
def get_translated_module(arg: str | int | typing.Any) -> str:  # frame not represented as hint
    ...
    if isinstance(arg, str):
        if arg.startswith('eden.addons.'):
            # get the name of the module
            return arg.split('.')[2]
        if '.' in arg or not arg:
            # module name is not in eden.addons.
            return 'base'
        else:
            return arg
    else:
        ...
        if not frame:
            return 'base'
        if (module_name := frame.f_globals.get("__name__")) and module_name.startswith('eden.addons.'):
            # just a quick lookup because `get_resource_from_path is slow compared to this`
            return module_name.split('.')[2]
        path = inspect.getfile(frame)
        path_info = eden.modules.get_resource_from_path(path)
...
def _get_cr(frame):
    ...
    if (local_self := frame.f_locals.get('self')) is not None:
        ...
        if (cr := getattr(local_self, 'cr', None)) is not None:
            return cr
    try:
        from eden.http import request  # noqa: PLC0415
...
def _get_lang(frame, default_lang='') -> str:
    ...
    uid = _get_uid(frame)
    if cr and uid:
        env = eden.api.Environment(cr, uid, {})
...
def TranslationFileReader(source, fileformat='po'):
    """ Iterate over translation file to return Eden translation entries """
    if fileformat == 'csv':
...
class PoFileReader:
    """ Iterate over po file to return Eden translation entries """
    def __init__(self, source):
...
class TranslationReader:
    def __init__(self, cr, lang=None):
        ...
        self.env = eden.api.Environment(cr, eden.SUPERUSER_ID, {})
        self._to_translate = []
...
class TranslationModuleReader(TranslationReader):
    ...
    def __init__(self, cr, modules=None, lang=None):
        ...
        self._path_list = [(path, True) for path in eden.addons.__path__]
            ...
            for root, dummy, files in os.walk(path, followlinks=True):
                ...
                if fnmatch.fnmatch(root, '*/static/src*'):
                    ...
                    for fname in fnmatch.filter(files, '*.xml'):
                        self._babel_extract_terms(fname, path, root, 'eden.tools.translate:babel_extract_qweb',
                                                  extra_comments=[JAVASCRIPT_TRANSLATION_COMMENT])
                if fnmatch.fnmatch(root, '*/data/*'):
                    for fname in filter(spreadsheet_files_regex.match, files):
                        self._babel_extract_terms(fname, path, root, 'eden.tools.translate:extract_spreadsheet_terms',
                                                  extra_comments=[JAVASCRIPT_TRANSLATION_COMMENT])
                if not recursive:
...
def get_po_paths(module_name: str, lang: str, env: eden.api.Environment | None = None):
...
def _get_translation_upgrade_queries(cr, field):
    ...
    """
    from eden.modules.registry import Registry  # noqa: PLC0415
