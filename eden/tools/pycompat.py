...
def csv_reader(stream, **params):
    warnings.warn("Deprecated since Eden 18.0: can just use `csv.reader` with a text stream or use `TextIOWriter` or `codec.getreader` to transcode.", DeprecationWarning, 2)
...
def csv_writer(stream, **params):
    warnings.warn("Deprecated since Eden 18.0: can just use `csv.writer` with a text stream or use `TextIOWriter` or `codec.getwriter` to transcode.", DeprecationWarning, 2)
    assert not isinstance(stream, io.TextIOBase), \
...
def to_text(source: typing.Any) -> str:
    ...
    * rest is textified
    """
    warnings.warn("Deprecated since Eden 18.0.", DeprecationWarning, 2)
