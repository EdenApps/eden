...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        self.options = {
            'admin_passwd': 'admin',
            'csv_internal_sep': ',',
            'publisher_warranty_url': 'http://services.edencloud.us/publisher-warranty/',
            'reportgz': False,
            'root_path': None,
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.edenrc (or to ~/.openerp_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store Eden data")
        ...
        group.add_option("-p", "--http-port", dest="http_port", my_default=8042,
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'eden_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "eden.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="eden.http:DEBUG", help='shortcut for --log-handler=eden.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="eden.sql_db:DEBUG", help='shortcut for --log-handler=eden.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate Eden to another language. "
    ...
    def parse_config(self, args: list[str] | None = None, *, setup_logging: bool | None = None) -> None:
        ...
        This method initializes eden.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            eden.tools.config.parse_config(sys.argv[1:])
        """
        opt = self._parse_config(args)
        if setup_logging is not False:
            eden.netsvc.init_logger()
            ...
            if setup_logging is None:
                warnings.warn(
                    "As of Eden 18, it's recommended to specify whether"
                    " you want Eden to setup its own logging (or want to"
                    " handle it yourself)",
                    category=PendingDeprecationWarning,
                    stacklevel=2,
                )
        self._warn_deprecated_options()
        eden.modules.module.initialize_sys_path()
        return opt
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'eden.conf')
        else:
            rcfilepath = os.path.expanduser('~/.edenrc')
            old_rcfilepath = os.path.expanduser('~/.openerp_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.edenrc' and '.openerp_serverrc' in your path. Please keep only one of "\
                "them, preferably '.edenrc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('EDEN_RC') or os.environ.get('OPENERP_SERVER') or rcfilepath)
        self.load()
    ...
    def _is_addons_path(self, path):
        from eden.modules.module import MANIFEST_NAMES
