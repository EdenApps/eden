...
from eden.tools.arabic_reshaper import reshape
from eden.tools.parse_version import parse_version
...
def add_banner(pdf_stream, text=None, logo=False, thickness=2 * cm):
    """ Add a banner on a PDF in the upper right corner, with Eden's logo (optionally).

    :param pdf_stream (BytesIO):    The PDF stream where the banner will be applied.
    :param text (str):              The text to be displayed.
    :param logo (bool):             Whether to display Eden's logo in the banner.
    ...
    eden_logo = Image.open(file_open('base/static/img/main_partner-image.png', mode='rb'))
    eden_color = colors.Color(113 / 255, 75 / 255, 103 / 255, 0.8)
    ...
    for p in range(old_pdf.getNumPages()):
        ...
        logo and can.drawImage(
            ImageReader(eden_logo), 0.25 * thickness, -2.05 * thickness, 40, 40, mask='auto', preserveAspectRatio=True)
...
class EdenPdfFileReader(PdfFileReader):
...
class EdenPdfFileWriter(PdfFileWriter):
    ...
    def embed_eden_attachment(self, attachment, subtype=None):
        assert attachment, "embed_eden_attachment cannot be called without attachment."
