...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False, filename=None):
    ...
    except eden.exceptions.UserError:
        raise
    except eden.exceptions.RedirectWarning:
        raise
...
`eden.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `eden.tools.safe_eval`.
""")
    return d
...
from eden._monkeypatches.pytz import patch_pytz  # noqa: E402, F401
patch_pytz()
