...
class Neutralize(Command):
    ...
    def run(self, args):
        ...
        opt = eden.tools.config.parse_config(args, setup_logging=True)

        dbname = eden.tools.config['db_name']
        if not dbname:
        ...
        try:
            with eden.sql_db.db_connect(dbname).cursor() as cursor:
                if opt.to_stdout:
                    installed_modules = eden.modules.neutralize.get_installed_modules(cursor)
                    queries = eden.modules.neutralize.get_neutralization_queries(installed_modules)
                    ...
                    eden.modules.neutralize.neutralize_database(cursor)
