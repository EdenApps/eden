...
class Obfuscate(Command):
    """Obfuscate data in a given eden database"""
    def __init__(self):
        super().__init__()
        self.cr = None
    ...
    def set_pwd(self, pwd):
        ...
        self.cr.execute("INSERT INTO ir_config_parameter (key, value) VALUES ('eden_cyph_pwd', 'eden_cyph_'||encode(pgp_sym_encrypt(%s, %s), 'base64')) ON CONFLICT(key) DO NOTHING", [pwd, pwd])
    ...
    def check_pwd(self, pwd):
        ...
        try:
            query = SQL("SELECT %s FROM ir_config_parameter WHERE key='eden_cyph_pwd'", uncypher_pwd)
    ...
    def clear_pwd(self):
        ...
        self.cr.execute("DELETE FROM ir_config_parameter WHERE key='eden_cyph_pwd' ")

    def cypher_string(self, sql_field: SQL, password):
        # don't double cypher fields
        return SQL("""CASE WHEN starts_with(%(field_name)s, 'eden_cyph_') THEN %(field_name)s ELSE 'eden_cyph_'||encode(pgp_sym_encrypt(%(field_name)s, %(pwd)s), 'base64') END""", field_name=sql_field, pwd=password)

    def uncypher_string(self, sql_field: SQL, password):
        return SQL("""CASE WHEN starts_with(%(field_name)s, 'eden_cyph_') THEN pgp_sym_decrypt(decode(substring(%(field_name)s, 11)::text, 'base64'), %(pwd)s) ELSE %(field_name)s END""", field_name=sql_field, pwd=password)
    ...
    def run(self, cmdargs):
        parser = eden.tools.config.parser
        ...
        try:
            opt = eden.tools.config.parse_config(cmdargs, setup_logging=True)
            if not opt.pwd:
            ...
            self.dbname = eden.tools.config['db_name']
