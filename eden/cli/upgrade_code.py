...
* addon: the eden addon in which the file is;
...
try:
    ...
    from eden.modules import initialize_sys_path
    from eden.tools import config, parse_version
except ImportError:
    # Assume the script is directy executed (by opposition to be
    # executed via eden-bin), happily release/parse_version are
    # standalone so we can hack our way there without importing eden
...
class UpgradeCode(Command):
    """ Rewrite the entire source code using the scripts found at /eden/upgrade_code """
    name = 'upgrade_code'
    ...
    def __init__(self):
        self.parser = argparse.ArgumentParser(
            ...
            description=__doc__.replace('/eden/upgrade_code', str(UPGRADE)),
    ...
    def run(self, cmdargs):
        ...
        if initialize_sys_path:
            ...
            initialize_sys_path()
            options.addons_path = eden.addons.__path__
        else:
