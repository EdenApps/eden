...
It will initialize the `eden.evented` variable.
"""
import eden
import sys
...
def patch_evented():
    if eden.evented or not (len(sys.argv) > 1 and sys.argv[1] == 'gevent'):
        return
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in eden/debian/copyright)
        while 1:
            state = conn.poll()
