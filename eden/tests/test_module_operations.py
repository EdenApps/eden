...
import eden
from eden.tools import config, topological_sort, unique
from eden.modules.registry import Registry
from eden.netsvc import init_logger
from eden.tests import standalone_tests
import eden.tests.loader

_logger = logging.getLogger('eden.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of Eden modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra Eden modules")
...
def test_standalone(args):
    ...
    for module_name in registry._init_modules:
        ...
        eden.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        eden.tools.config['addons_path'] = ','.join([args.addons_path, eden.tools.config['addons_path']])
        if args.data_dir:
            eden.tools.config['data_dir'] = args.data_dir
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'eden.modules.loading': {'level': 'CRITICAL'},
            'eden.sql_db': {'level': 'CRITICAL'},
            'eden.models.unlink': {'level': 'WARNING'},
            'eden.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
