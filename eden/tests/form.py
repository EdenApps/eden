# -*- coding: utf-8 -*-
"""
The module :mod:`eden.tests.form` provides an implementation of a client form
...
import eden
from eden.models import BaseModel
...
class Form:
    ...
    * properly handle defaults & onchanges around x2many fields.
    ...
    of :class:`~eden.fields.Many2one` fields, one can assign a recordset::
    ...
    For :class:`~eden.fields.Many2many` fields, the field itself is a
    :class:`~eden.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~eden.fields.One2many` are reified as :class:`~O2MProxy`.

    Because the :class:`~eden.fields.One2many` only exists through its parent,
    ...
    @classmethod
    def from_action(cls, env: eden.api.Environment, action: dict) -> Form:
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~eden.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~eden.fields.One2many` record.
...
def convert_read_to_form(values, fields):
    ...
    for fname, value in values.items():
        ...
        if field_info['type'] == 'one2many':
            ...
            value = eden.fields.Datetime.to_string(value)
        elif field_info['type'] == 'date' and isinstance(value, date):
            value = eden.fields.Date.to_string(value)
        result[fname] = value
    return result
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return eden.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return eden.fields.Date.to_string(value)
    return value
