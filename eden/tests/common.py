# -*- coding: utf-8 -*-
"""
The module :mod:`eden.tests.common` provides unittest test cases and a few
...
from eden.service import security
from eden.sql_db import BaseCursor, Cursor
from eden.tools import config, float_compare, mute_logger, profiler, SQL, DotDict
from eden.tools.mail import single_email_re
from eden.tools.misc import find_in_path, lower_logging
from eden.tools.xml_utils import _validate_xml
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: eden.tests.common.Form is deprecated, use eden.tests.Form",
...
# The eden library is supposed already configured.
ADDONS_PATH = eden.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = eden.SUPERUSER_ID
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding Eden module name.
    """
    def register(func):
        # register func by eden module name
        if func.__module__.startswith('eden.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most eden operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('eden.addons.'):
...
class BaseCase(case.TestCase, metaclass=MetaCase):
    """ Subclass of TestCase for Eden-specific code. This class is abstract and
    expects self.registry, self.cr and self.uid to be initialized by subclasses.
    """

    longMessage = True      # more verbose error message by default: https://www.edencloud.us/r/Vmh
    warm = True             # False during warm-up phase (see :func:`warmup`)
    _python_version = sys.version_info

    _tests_run_count = int(os.environ.get('EDEN_TEST_FAILURE_RETRIES', 0)) + 1
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~eden.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(eden.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            self.env.invalidate_all()
            eden.http._request_stack.push(request)
            yield
            self.env.flush_all()
            self.env.invalidate_all()
        finally:
            popped_request = eden.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('eden.sql_db.Cursor.execute', execute),
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/eden/addons/" in filename:
                        filename = filename.rsplit("/eden/addons/", 1)[1]
                    if count > expected:
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | eden.addons.base.models.res_currency.Currency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    cr: Cursor = None
    muted_registry_logger = mute_logger(eden.modules.registry._logger.name)
    freeze_time = None
    ...
    def _gc_filestore(cls):
        ...
        with Registry(get_db_name()).cursor() as cr:
            gc_env = api.Environment(cr, eden.SUPERUSER_ID, {})
    ...
    def setUpClass(cls):
        ...
        cls.env = api.Environment(cls.cr, eden.SUPERUSER_ID, {})
        ...
        cls._crypt_context_patcher = patch('eden.addons.base.models.res_users.Users._crypt_context', _crypt_context)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f"):
    ...
    screenshots_dir = pathlib.Path(eden.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
class ChromeBrowser:
    ...
    def __init__(self, test_case: HttpCase, success_signal: str = DEFAULT_SUCCESS_SIGNAL, headless: bool = True, debug: bool = False):
        ...
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_eden')

        otc = eden.tools.config
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way Eden widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    @classmethod
    def http_port(cls):
        if eden.service.server.server is None:
            return None
        return eden.service.server.server.httpd.server_port
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('eden.service.http.request.')]
        ...
        if request_threads:
            ...
            eden.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        eden.http.root.session_store.save(self.session)

    def authenticate(self, user, password, browser: ChromeBrowser = None):
        if getattr(self, 'session', None):
            eden.http.root.session_store.delete(self.session)

        self.session = session = eden.http.root.session_store.new()
        session.update(eden.http.get_default_session(), db=get_db_name())
        session.context['lang'] = eden.http.DEFAULT_LANG
        ...
        if user: # if authenticated
            ...
            with patch('eden.addons.base.models.res_users.Users._check_credentials', new=patched_check_credentials):
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        options = {
            ...
            'startUrl': url_path,
            'delayToCheckUndeterminisms': kwargs.pop('delay_to_check_undeterminisms', int(os.getenv("EDEN_TOUR_DELAY_TO_CHECK_UNDETERMINISMS", "0")) or 0),
        }
        code = kwargs.pop('code', f"eden.startTour({tour_name!r}, {json.dumps(options)})")
        ready = kwargs.pop('ready', f"eden.isTourReady({tour_name!r})")
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('eden.http.Request._get_profiler_context_manager', route_profiler))
...
def tagged(*tags):
    ...
    By default, all Test classes from eden.tests.common have a test_tags
...
class freeze_time:
    """ Object to replace the freezegun in Eden test suites
