...
from .suite import EdenSuite
from .result import EdenTestResult
...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'eden.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"eden.upgrade.{module}",
        f"eden.addons.{module}.migrations",
        f"eden.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return EdenSuite(sorted(tests, key=lambda t: t.test_sequence))
