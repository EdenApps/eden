...
from eden.exceptions import CacheMiss
from eden.osv import expression

import typing
from eden.api import ContextType, DomainType, IdType, NewId, M, T
...
class Field(MetaField('DummyField', (object,), {}), typing.Generic[T]):
    ...
    :param str aggregator: aggregate function used by :meth:`~eden.models.Model.read_group`
    ...
    def _get_attrs(self, model_class, name):
        ...
        if 'group_operator' in attrs:
            warnings.warn("Since Eden 18, 'group_operator' is deprecated, use 'aggregator' instead", DeprecationWarning, 2)
    ...
    def mapped(self, records):
        ...
        over a simple call to `~eden.models.BaseModel.mapped()` on a recordset.
        """
        if self.name == 'id':
            # not stored in cache
...
class Float(Field[float]):
    ...
    :param digits: a pair (total, decimal) or a string referencing a
        :class:`~eden.addons.base.models.decimal_precision.DecimalPrecision` record name.
    ...
    :func:`~eden.fields.Float.round()` to round a float with the given precision.
    :func:`~eden.fields.Float.is_zero()` to check if a float equals zero at the given precision.
    :func:`~eden.fields.Float.compare()` to compare two floats at the given precision.
...
class Monetary(Field[float]):
    ...
    :class:`res_currency<eden.addons.base.models.res_currency.Currency>`.
    ...
    :param str currency_field: name of the :class:`Many2one` field
        holding the :class:`res_currency <eden.addons.base.models.res_currency.Currency>`
    ...
    def convert_to_cache(self, value, record, validate=True):
        ...
        if value and validate:
            # FIXME @rco-eden: currency may not be already initialized if it is
...
class Html(_String):
    ...
    def _get_attrs(self, model_class, name):
        ...
        # we do not need this Microsoft Outlook client feature for emails displayed Eden's web client.
...
class Image(Binary):
    ...
    :param bool verify_resolution: whether the image resolution should be verified
        ...
        See :class:`eden.tools.image.ImageProcess` for maximum image resolution (default: ``50e6``).
...
class Many2one(_Relational[M]):
    ...
    :param bool check_company: Mark the field to be verified in
        :meth:`~eden.models.Model._check_company`. Has a different behaviour
...
class Properties(Field):
    ...
    like Eden fields but without being independently stored in database.
    ...
    current record. The container must have a :class:`~eden.fields.PropertiesDefinition`
...
class PropertiesDefinition(Field):
    """ Field used to define the properties definition (see :class:`~eden.fields.Properties`
...
class Command(enum.IntEnum):
    """
    :class:`~eden.fields.One2many` and :class:`~eden.fields.Many2many` fields
    ...
    def create(cls, values: dict):
        ...
        In case of a :class:`~eden.fields.Many2many` relation, one unique
        new record is created in the comodel such that all records in `self`
        are linked to the new record.

        In case of a :class:`~eden.fields.One2many` relation, one new record
    ...
    def delete(cls, id: int):
        ...
        In case of a :class:`~eden.fields.Many2many` relation, removing the
    ...
    def unlink(cls, id: int):
        ...
        In case of a :class:`~eden.fields.One2many` relation, the given record
...
class Many2many(_RelationalMulti[M]):
    ...
    :param bool check_company: Mark the field to be verified in
        :meth:`~eden.models.Model._check_company`. Add a default company
