...
class Cursor(BaseCursor):
    ...
    def execute_values(self, query, argslist, template=None, page_size=100, fetch=False):
        ...
        """
        # Eden Cursor only proxies all methods of psycopg2 Cursor. This is a patch for problems caused by passing
...
def connection_info_for(db_or_uri, readonly=False):
    ...
    :rtype: (str, dict)
    """
    if 'EDEN_PGAPPNAME' in os.environ:
        # Using manual string interpolation for security reason and trimming at default NAMEDATALEN=63
        app_name = os.environ['EDEN_PGAPPNAME'].replace('{pid}', str(os.getpid()))[0:63]
    else:
        app_name = "eden-%d" % os.getpid()
...
def db_connect(to, allow_uri=False, readonly=False):
    ...
    maxconn = eden.evented and tools.config['db_maxconn_gevent'] or tools.config['db_maxconn']
...
def close_db(db_name):
    """ You might want to call eden.modules.registry.Registry.delete(db_name) along this function."""
    if _Pool:
