...
!ifndef SERVICENAME
	!define SERVICENAME 'eden-server-${VERSION}'
!endif

!ifndef TOOLSDIR
	!define TOOLSDIR 'c:\edenbuild'
!endif
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\Eden Server ${VERSION}"
...
OutFile "${TOOLSDIR}\server\eden_setup_${VERSION}.exe"
...
!define MUI_ICON "${PIXMAPS_PATH}\eden-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\eden-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\eden-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\eden-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.edencloud.us/page/contactus"
...
; English
LangString DESC_Eden_Server ${LANG_ENGLISH} "Install the Eden Server with all the Eden standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by Eden."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact Eden for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the Eden Server"
...
LangString Profile_AllInOne ${LANG_ENGLISH} "Eden Server And PostgreSQL Server"
LangString Profile_Server ${LANG_ENGLISH} "Eden Server Only"
LangString Profile_IOT ${LANG_ENGLISH} "Eden IoT"
LangString TITLE_Eden_Server ${LANG_ENGLISH} "Eden Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString TITLE_IOT ${LANG_ENGLISH} "Eden IoT"
LangString TITLE_Nginx ${LANG_ENGLISH} "Nginx WebServer"
LangString TITLE_Ghostscript ${LANG_ENGLISH} "Ghostscript interpreter"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start Eden"
LangString UnsafeDirText ${LANG_ENGLISH} "Installing outside of $PROGRAMFILES64 is not recommended.$\nDo you want to continue ?"

; French
LangString DESC_Eden_Server ${LANG_FRENCH} "Installation du Serveur Eden avec tous les modules Eden standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par Eden."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez Eden pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur Eden"
...
LangString Profile_AllInOne ${LANG_FRENCH} "Serveur Eden Et Serveur PostgreSQL"
LangString Profile_Server ${LANG_FRENCH} "Seulement Le Serveur Eden"
LangString Profile_IOT ${LANG_FRENCH} "Eden IoT"
LangString TITLE_Eden_Server ${LANG_FRENCH} "Serveur Eden"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString TITLE_IOT ${LANG_FRENCH} "Eden IoT"
...
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer Eden"
...
Section $(TITLE_Eden_Server) SectionEden_Server
    SectionIn 1 2 3
    ...
    # If there is a previous install of the Eden Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\eden.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\eden.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\eden.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\eden.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\eden.conf" "options" "addons_path" "$INSTDIR\server\eden\addons"
    WriteIniStr "$INSTDIR\server\eden.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    # Set data_dir
    WriteIniStr "$INSTDIR\server\eden.conf" "options" "data_dir" "$INSTDIR\sessions"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\eden.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    # Productivity Apps
    WriteIniStr "$INSTDIR\server\eden.conf" "options" "default_productivity_apps" "True"
    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\eden-bin" --stop-after-init -c "$INSTDIR\server\eden.conf" --logfile "$INSTDIR\server\eden.log" -s'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppDirectory "$\"$INSTDIR\python$\""'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\server\eden-bin\" -c "\"$INSTDIR\server\eden.conf\"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} ObjectName "LOCALSERVICE"'
    AccessControl::GrantOnFile  "$INSTDIR" "LOCALSERVICE" "FullAccess"

    Call RestartEdenService
SectionEnd
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\$postgresql_exe_filename" \
        ...
        --servicename "PostgreSQL_For_Eden" \
...
Section $(TITLE_IOT) IOT
    ...
    WriteIniStr "$INSTDIR\server\eden.conf" "options" "server_wide_modules" "web,hw_posbox_homepage,hw_drivers"
    WriteIniStr "$INSTDIR\server\eden.conf" "options" "list_db" "False"
    WriteIniStr "$INSTDIR\server\eden.conf" "options" "max_cron_threads" "0"
    nsExec::ExecToStack '"$INSTDIR\python\python.exe" "$INSTDIR\server\eden-bin" genproxytoken'
    pop $0
    pop $ProxyTokenPwd
SectionEnd
...
Section $(TITLE_Nginx) Nginx
    ...
    File "..\..\eden\addons\point_of_sale\tools\posbox\overwrite_after_init\etc\ssl\certs\nginx-cert.crt"
    File "..\..\eden\addons\point_of_sale\tools\posbox\overwrite_after_init\etc\ssl\private\nginx-cert.key"
SectionEnd
...
Section -Post
    ...
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@edencloud.us"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.edencloud.us"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@edencloud.us"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionEden_Server} $(DESC_Eden_Server)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        StrCpy $INSTDIR "$PROGRAMFILES64\Eden ${VERSION}"
...
Function ShowProxyTokenDialogPage
    ...
    doProxyToken:
        ...
        ${NSD_CreateLabel} 0 0 100% 25% "Here is your access token for the Eden IOT, please write it down in a safe place, you will need it to configure the IOT"
        Pop $ProxyTokenLabel
...
Function ComponentLeave
    SectionGetFlags ${SectionEden_Server} $0
...
Function RestartEdenService
    DetailPrint "Restarting Eden Service"
