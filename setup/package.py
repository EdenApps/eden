...
version_info = ...
nt_service_name = ...
exec(open(os.path.join(ROOTDIR, 'eden', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/eden && \
    groupadd -g %(group_id)s eden && \
    useradd -u %(user_id)s -g eden eden -d /var/lib/eden && \
    mkdir /data && \
    chown eden:eden /var/lib/eden /data
USER eden
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class EdenTestTimeoutError(Exception):
    pass


class EdenTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise EdenTestError("Installation of package failed")
        else:
        ...
        raise EdenTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/eden_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False, move_addons=True):
    ...
    run_cmd(cmd + ['%s/' % args.eden_dir, args.build_dir])
    if not move_addons:
        return
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'eden/addons'))
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'eden-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
        ...
        self.docker_template = Path(docker_templates[self.arch]).read_text(encoding='utf-8').replace('USER eden', DOCKERUSER)
    ...
    def run(self, cmd, build_dir, container_name, user='eden', exposed_port=None, detach=False, timeout=None):
    ...
    def test_eden(self):
        logging.info('Starting to test Eden install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)
            if os.path.exists(os.path.join(args.build_dir, 'eden.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise EdenTestTimeoutError('Eden pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise EdenTestError('Error while installing/starting Eden after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'eden-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/eden-*.tar.gz' % self.args.build_dir)[0], '%s/eden_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/eden-*.zip' % self.args.build_dir)[0], '%s/eden_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su postgres -s /bin/bash -c "createuser -s eden"',
            'su eden -s /bin/bash -c "python3 -m venv /var/lib/eden/edenvenv"',
            'su eden -s /bin/bash -c "/var/lib/eden/edenvenv/bin/python3 -m pip install --upgrade pip"',
            'su eden -s /bin/bash -c "/var/lib/eden/edenvenv/bin/python3 -m pip install -r /opt/release/requirements.txt"',
            f'su eden -s /bin/bash -c "/var/lib/eden/edenvenv/bin/python3 -m pip install /data/src/eden_{VERSION}.{TSTAMP}.tar.gz"',
            'su eden -s /bin/bash -c "createdb mycompany"',
            'su eden -s /bin/bash -c "/var/lib/eden/edenvenv/bin/eden -d mycompany -i base --stop-after-init"',
            'su eden -s /bin/bash -c "/var/lib/eden/edenvenv/bin/eden -d mycompany --pidfile=/data/src/eden.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'eden-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_eden()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/eden (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../eden_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'eden-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            f'/usr/bin/apt-get install -y /data/src/eden_{VERSION}.{TSTAMP}_all.deb',
            'su eden -s /bin/bash -c "eden -d mycompany -i base --pidfile=/data/src/eden.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'eden-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_eden()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        rpmbuild_dir = '/var/lib/eden/rpmbuild'
        cmds = [
            'cd /data/src',
            'mkdir -p dist',
            'rpmdev-setuptree -d',
            f'cp -a /data/src/setup/rpm/eden.spec {rpmbuild_dir}/SPECS/',
            f'tar --transform "s/^\\./eden-{VERSION}/" -c -z -f {rpmbuild_dir}/SOURCES/eden-{VERSION}.tar.gz .',
            f'rpmbuild -bb --define="%version {VERSION}" /data/src/setup/rpm/eden.spec',
            f'mv {rpmbuild_dir}/RPMS/noarch/eden*.rpm /data/src/dist/'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, f'eden-rpm-build-{TSTAMP}')
        os.rename(glob('%s/dist/eden-*.noarch.rpm' % self.args.build_dir)[0], '%s/eden_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'sleep 5',
            'su postgres -c "createuser -s eden"',
            'su eden -c "createdb mycompany"',
            'dnf install -d 0 -e 0 /data/src/eden_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su eden -s /bin/bash -c "eden -c /etc/eden/eden.conf -d mycompany -i base --stop-after-init"',
            'su eden -s /bin/bash -c "eden -c /etc/eden/eden.conf -d mycompany --pidfile=/data/src/eden.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'eden-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_eden()
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'eden-rpm-createrepo-%s' % TSTAMP)
...
class DockerWine(Docker):
    ...
    def build(self):
        ...
        container_python = '/var/lib/eden/.wine/drive_c/edenbuild/WinPy64/python-3.12.3.amd64/python.exe'
        ...
        cmds = [
            ...
            rf'wine "c:\nsis-3.10\makensis.exe" {nsis_args} "c:\edenbuild\server\setup\win32\setup.nsi"',
            rf'wine {container_python} -m pip list'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'eden-win-build-%s' % TSTAMP)
