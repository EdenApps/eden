...
test("Basic evaluation", async () => {
    ...
    setCellContent(model, "A1", `=EDEN.PARTNER.BALANCE("14, 16", "112", 2023)`);
...
test("with wrong date format", async () => {
    const model = await createModelWithDataSource();
    setCellContent(model, "A1", `=EDEN.PARTNER.BALANCE("14, 16", "112", "This is not a valid date")`);
    await waitForDataLoaded(model);
...
test("with no date", async () => {
    ...
    setCellContent(model, "A1", `=EDEN.PARTNER.BALANCE("14, 16", "112")`);
    await waitForDataLoaded(model);
    expect.verifySteps(["spreadsheet_fetch_partner_balance"]);
    expect(getCellValue(model, "A1")).toBe(26);
});
