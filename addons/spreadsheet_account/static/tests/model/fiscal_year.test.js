...
test("Basic evaluation", async () => {
    ...
    setCellContent(model, "A1", `=EDEN.FISCALYEAR.START("11/11/2020")`);
    setCellContent(model, "A2", `=EDEN.FISCALYEAR.END("11/11/2020")`);
...
test("with a given company id", async () => {
    ...
    setCellContent(model, "A1", `=EDEN.FISCALYEAR.START("11/11/2020", 1)`);
    setCellContent(model, "A2", `=EDEN.FISCALYEAR.END("11/11/2020", 1)`);
    await waitForDataLoaded(model);
    expect.verifySteps(["get_fiscal_dates"]);
    expect(getEvaluatedCell(model, "A1").formattedValue).toBe("1/1/2020");
    expect(getEvaluatedCell(model, "A2").formattedValue).toBe("12/31/2020");
...
test("with a wrong company id", async () => {
    ...
    setCellContent(model, "A1", `=EDEN.FISCALYEAR.START("11/11/2020", 100)`);
    setCellContent(model, "A2", `=EDEN.FISCALYEAR.END("11/11/2020", 100)`);
    await waitForDataLoaded(model);
    expect.verifySteps(["get_fiscal_dates"]);
...
test("with wrong input arguments", async () => {
    const model = await createModelWithDataSource();
    setCellContent(model, "A1", `=EDEN.FISCALYEAR.START("not a number")`);
    setCellContent(model, "A2", `=EDEN.FISCALYEAR.END("11/11/2020", "not a number")`);
    expect(getEvaluatedCell(model, "A1").message).toBe(
        "The function EDEN.FISCALYEAR.START expects a number value, but 'not a number' is a string, and cannot be coerced to a number."
    );
    expect(getEvaluatedCell(model, "A2").message).toBe(
        "The function EDEN.FISCALYEAR.END expects a number value, but 'not a number' is a string, and cannot be coerced to a number."
    );
});
