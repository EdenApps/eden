...
from eden.tests import Form, HttpCase, tagged, users
from eden.tools import convert_file
...
class TestMailTemplate(MailCommon):
    ...
    def test_mail_template_acl(self):
        ...
        for expression in allowed_qweb_expressions:
            ...
            with (patch('eden.addons.base.models.ir_qweb.IrQWeb._render', side_effect=o_qweb_render) as qweb_render,
                patch('eden.addons.base.models.ir_qweb.unsafe_eval', side_effect=eval) as unsafe_eval):
        ...
        with patch('eden.tools.safe_eval.unsafe_eval', side_effect=eval) as unsafe_eval:
        ...
        for template, excepted in templates:
            ...
            with patch('eden.addons.mail.models.mail_render_mixin.MailRenderMixin._render_template_qweb_regex', side_effect=o_render) as render:
                rendered = mail_template._render_field('body_html', record.ids)[record.id]
...
class TestMailTemplateReset(MailCommon):
    ...
    def test_mail_template_reset(self):
        ...
        self.assertEqual(mail_template.body_html.strip(), Markup('<div>Hello Eden</div>'))
        ...
        def fake_load_file(translation_importer, filepath, lang, xmlids=None):
            ...
            if lang == 'fr_FR':  # fr_FR has translations
                translation_importer.model_translations['mail.template'] = {
                    'body_html': {'mail.mail_template_test': {'fr_FR': '<div>Hello Eden FR</div>'}},
                    'name':  {'mail.mail_template_test': {'fr_FR': "Mail: Test Mail Template FR"}},
                }

        with patch('eden.tools.translate.TranslationImporter.load_file', fake_load_file):
            mail_template_reset = self.env['mail.template.reset'].with_context(context).create({})
            reset_action = mail_template_reset.reset_template()
        self.assertTrue(reset_action)

        self.assertEqual(mail_template.body_html.strip(), Markup('<div>Hello Eden</div>'))
        self.assertEqual(mail_template.with_context(lang='en_GB').body_html.strip(), Markup('<div>Hello Eden</div>'))
        self.assertEqual(mail_template.with_context(lang='fr_FR').body_html.strip(), Markup('<div>Hello Eden FR</div>'))
...
class TestMailTemplateUI(HttpCase):
    ...
    def test_mail_template_dynamic_placeholder_tour(self):
        self.start_tour("/eden", 'mail_template_dynamic_placeholder_tour', login="admin")
