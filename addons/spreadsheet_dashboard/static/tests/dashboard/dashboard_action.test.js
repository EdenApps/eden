...
test("share dashboard from dashboard view", async function () {
    patchWithCleanup(browser.navigator.clipboard, {
        writeText: (url) => {
            ...
            expect(url).toBe("localhost:8042/share/url/132465");
        },
    });
    ...
    await createSpreadsheetDashboard({
        mockRPC: async function (route, args) {
            if (args.method === "action_get_share_url") {
                ...
                return "localhost:8042/share/url/132465";
            }
        },
    });
    ...
    expect(".o_field_CopyClipboardChar").toHaveText("localhost:8042/share/url/132465");
...
test("Changing filter values will create a new share", async function () {
    ...
    await createSpreadsheetDashboard({
        ...
        mockRPC: async function (route, args) {
            if (args.method === "action_get_share_url") {
                return `localhost:8042/share/url/${++counter}`;
            }
        },
    });
    await contains("i.fa-share-alt").click();
    await animationFrame();
    expect(".o_field_CopyClipboardChar").toHaveText(`localhost:8042/share/url/1`);
    ...
    await contains("i.fa-share-alt").click();
    await animationFrame();
    expect(".o_field_CopyClipboardChar").toHaveText(`localhost:8042/share/url/2`);
});
