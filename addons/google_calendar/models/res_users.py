...
from eden.addons.google_calendar.utils.google_calendar import GoogleCalendarService, InvalidSyncToken
from eden.addons.google_calendar.models.google_sync import google_calendar_token
from eden.addons.google_account.models import google_service
from eden.loglevels import exception_to_unicode
...
class User(models.Model):
    ...
    def _check_pending_eden_records(self):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        if not results or (not results.get('events') and not self._check_pending_eden_records()):
            return False
        ...
        # We apply Google updates only if their write date is later than the write date in Eden.
        # It's possible that multiple updates affect the same record, maybe not directly.
        # To handle this, we preserve the write dates in Eden before applying any updates,
        # and use these dates instead of the current live dates.
        eden_events = self.env['calendar.event'].browse((events - recurrences).eden_ids(self.env))
        eden_recurrences = self.env['calendar.recurrence'].browse(recurrences.eden_ids(self.env))
        recurrences_write_dates = {r.id: r.write_date for r in eden_recurrences}
        events_write_dates = {e.id: e.write_date for e in eden_events}
        synced_recurrences = self.env['calendar.recurrence']._sync_google2eden(recurrences, recurrences_write_dates)
        synced_events = self.env['calendar.event']._sync_google2eden(events - recurrences, events_write_dates, default_reminders=default_reminders)

        # Eden -> Google
        recurrences = self.env['calendar.recurrence']._get_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences.with_context(send_updates=send_updates)._sync_eden2google(calendar_service)
        ...
        (events - synced_events).with_context(send_updates=send_updates)._sync_eden2google(calendar_service)

        return bool(results) and (bool(events | synced_events) or bool(recurrences | synced_recurrences))

    def _sync_single_event(self, calendar_service: GoogleCalendarService, eden_event, event_id):
        self.ensure_one()
        ...
        synced_events = self.env['calendar.event']._sync_google2eden(event, default_reminders=default_reminders)
        # Eden -> Google
        eden_event.with_context(send_updates=send_updates)._sync_eden2google(calendar_service)
        return bool(eden_event | synced_events)
