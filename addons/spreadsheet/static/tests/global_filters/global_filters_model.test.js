...
import { animationFrame, mockDate, mockTimeZone } from "@eden/hoot-mock";

import { DispatchResult, Model, helpers, tokenize } from "@eden/o-spreadsheet";
...
test("Adding new DataSource will set its fieldMatching according to other ones with the same model", async function () {
    ...
    const chartId = model.getters.getEdenChartIds()[0];
    fieldMatching = model.getters.getEdenChartFieldMatching(chartId, filterId);
    expect(fieldMatching.chain).toBe("created_on");
    expect(fieldMatching.type).toBe("date");
    expect(fieldMatching.offset).toBe(undefined);
});
...
test("EDEN.FILTER.VALUE text filter", async function () {
    const model = await createModelWithDataSource();
    setCellContent(model, "A10", `=EDEN.FILTER.VALUE("Text Filter")`);
...
test("EDEN.FILTER.VALUE date filter", async function () {
    const model = await createModelWithDataSource();
    setCellContent(model, "A10", `=EDEN.FILTER.VALUE("Date Filter")`);
    await animationFrame();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
...
test("EDEN.FILTER.VALUE date from/to without values", async function () {
    const model = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
    ...
    setCellContent(model, "A1", `=EDEN.FILTER.VALUE("Date Filter")`);
    expect(getEvaluatedCell(model, "A1").value).toBe("");
    expect(getEvaluatedCell(model, "B1").value).toBe("");
});

test("EDEN.FILTER.VALUE date from/to with only from defined", async function () {
    const model = await createModelWithDataSource();
    setCellContent(model, "A1", `=EDEN.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("EDEN.FILTER.VALUE date from/to with only to defined", async function () {
    const model = await createModelWithDataSource();
    setCellContent(model, "A1", `=EDEN.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("EDEN.FILTER.VALUE date from/to with from and to defined", async function () {
    const model = await createModelWithDataSource();
    setCellContent(model, "A1", `=EDEN.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("EDEN.FILTER.VALUE relation filter", async function () {
    const model = await createModelWithDataSource({
    ...
    setCellContent(model, "A10", `=EDEN.FILTER.VALUE("Relation Filter")`);
    await animationFrame();
    await addGlobalFilter(model, {
        id: "42",
        type: "relation",
...
test("EDEN.FILTER.VALUE with escaped quotes in the filter label", async function () {
    const model = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "text",
    ...
    setCellContent(model, "A1", '=EDEN.FILTER.VALUE("my \\"special\\" filter")');
    expect(getCellValue(model, "A1")).toBe("Jean-Jacques");
});

test("EDEN.FILTER.VALUE formulas are updated when filter label is changed", async function () {
    const model = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
    ...
    setCellContent(
        model,
        "A10",
        `=EDEN.FILTER.VALUE("Cuillère") & EDEN.FILTER.VALUE( "Cuillère" )`
    );
    const [filter] = model.getters.getGlobalFilters();
    const newFilter = {
        ...filter,
    ...
    expect(getCellFormula(model, "A10")).toBe(
        `=EDEN.FILTER.VALUE("Interprete") & EDEN.FILTER.VALUE("Interprete")`
    );
});
...
test("field matching is removed when list is deleted", async function () {
    ...
    model.dispatch("REMOVE_EDEN_LIST", { listId });
...
test("field matching is removed when an Eden chart is deleted", async function () {
    ...
    expect(model.getters.getEdenChartFieldMatching(chartId, filter.id)).toEqual(matching);
    model.dispatch("DELETE_FIGURE", { id: chartId, sheetId });
    expect(() => model.getters.getEdenChartFieldMatching(chartId, filter.id)).toThrow(undefined, {
...
test("Updating the pivot domain should keep the global filter domain", async () => {
    ...
    model.dispatch("UPDATE_EDEN_PIVOT_DOMAIN", {
        pivotId,
        domain: [["foo", "in", [55]]],
    });
...
test("Updating a non-eden pivot should not crash on global filter", async () => {
    const grid = {
