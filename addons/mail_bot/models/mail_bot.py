...
from eden.tools import html_escape
...
class MailBot(models.AbstractModel):
    ...
    def _apply_logic(self, record, values, command=None):
        ...
        The logic will only be applied if edenbot is in a chat with a user or
        if someone pinged edenbot.

         :param record: the discuss channel where the user message was posted/edenbot will answer.
        ...
        if values.get("author_id") == edenbot_id or values.get("message_type") != "comment" and not command:
            return
    ...
    def _get_style_dict():
        return {
            ...
            "bold_start": Markup("<b>"),
            "bold_end": Markup("</b>"),
            "command_start": Markup("<span class='o_edenbot_command'>"),
            "command_end": Markup("</span>"),
            "document_link_start": Markup("<a href='https://www.edencloud.us/documentation' target='_blank'>"),
            "document_link_end": Markup("</a>"),
            "slides_link_start": Markup("<a href='https://www.edencloud.us/slides' target='_blank'>"),
    ...
    def _get_answer(self, record, body, values, command=False):
        ...
        # onboarding
        edenbot_state = self.env.user.edenbot_state

        if record.channel_type == "chat" and edenbot in record.channel_member_ids.partner_id:
            # main flow
            source = _("Thanks")
            description = _("This is a temporary canned response to see how canned responses work.")
            if edenbot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
                self.env.user.edenbot_state = "onboarding_command"
                self.env.user.edenbot_failed = False
                return html_escape(
            ...
            elif edenbot_state == 'onboarding_command' and command == 'help':
                self.env.user.edenbot_state = "onboarding_ping"
                self.env.user.edenbot_failed = False
                return html_escape(
                    _("Wow you are a natural!%(new_line)sPing someone with @username to grab their attention. "
                      "%(bold_start)sTry to ping me using%(bold_end)s %(command_start)s@EdenBot%(command_end)s"
                      " in a sentence.")) % self._get_style_dict()
            elif edenbot_state == "onboarding_ping" and edenbot.id in values.get("partner_ids", []):
                self.env.user.edenbot_state = "onboarding_attachement"
                self.env.user.edenbot_failed = False
                return html_escape(
            ...
            elif edenbot_state == "onboarding_attachement" and values.get("attachment_ids"):
                ...
                self.env.user.edenbot_failed = False
                self.env.user.edenbot_state = "onboarding_canned"
                return html_escape(
            ...
            elif edenbot_state == "onboarding_canned" and self.env.context.get("canned_response_ids"):
                ...
                self.env.user.edenbot_failed = False
                self.env.user.edenbot_state = "idle"
                return html_escape(
                    _("Good, you can customize canned responses in the Discuss application."
                      ...
                      " Enjoy discovering Eden!")) % self._get_style_dict()
            # repeat question if needed
            elif edenbot_state == 'onboarding_canned' and not self._is_help_requested(body):
                self.env.user.edenbot_failed = True
                return html_escape(
            ...
            elif edenbot_state in (False, "idle", "not_initialized") and (_('start the tour') in body.lower()):
                self.env.user.edenbot_state = "onboarding_emoji"
                return _("To start, try to send me an emoji :)")
            # easter eggs
            elif edenbot_state == "idle" and body in ['❤️', _('i love you'), _('love')]:
            ...
            # help message
            elif self._is_help_requested(body) or edenbot_state == 'idle':
                return html_escape(
                ...
                # repeat question
                if edenbot_state == 'onboarding_emoji':
                    self.env.user.edenbot_failed = True
                    return (html_escape(
                ...
                elif edenbot_state == 'onboarding_ping':
                    self.env.user.edenbot_failed = True
                    return html_escape(
                        _("Sorry, I am not listening. To get someone's attention, %(bold_start)sping him%(bold_end)s."
                          " Write %(command_start)s@EdenBot%(command_end)s and select me.")) % self._get_style_dict()
    ...
    def _is_bot_pinged(self, values):
        edenbot_id = self.env['ir.model.data']._xmlid_to_res_id("base.partner_root")
        return edenbot_id in values.get('partner_ids', [])

    def _is_bot_in_private_channel(self, record):
        edenbot_id = self.env['ir.model.data']._xmlid_to_res_id("base.partner_root")
        if record._name == 'discuss.channel' and record.channel_type == 'chat':
            return edenbot_id in record.with_context(active_test=False).channel_partner_ids.ids
        return False
    ...
    def _is_help_requested(self, body):
        ...
        """
        return any(token in body for token in ['help', _('help'), '?']) or self.env.user.edenbot_failed
