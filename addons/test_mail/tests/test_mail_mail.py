...
from eden import api, Command, fields, SUPERUSER_ID
...
from eden.tests import common, tagged, users
...
class TestMailMail(MailCommon):
    ...
    def test_mail_mail_send_exceptions_origin(self):
        ...
        for default_from in [False, '']:
            ...
            self._reset_data()
            with self.mock_mail_gateway(), mute_logger('eden.addons.mail.models.mail_mail'):
...
class TestMailMailServer(MailCommon):
    ...
    def test_mail_mail_values_headers(self):
        """ Test headers content, notably X-Eden-Message-Id added to keep context
    ...
    @mute_logger('eden.addons.mail.models.mail_mail')
    @patch('eden.addons.base.models.ir_attachment.IrAttachment.file_size', new_callable=PropertyMock)
...
class TestMailMailRace(common.TransactionCase):
    ...
    def test_mail_bounce_during_send(self):
        ...
        def send_email(self, message, *args, **kwargs):
            with this.registry.cursor() as cr, mute_logger('eden.sql_db'):
                try:
