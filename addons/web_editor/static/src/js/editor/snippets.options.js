...
const resetOuids = EdenEditorLib.resetOuids;
let _serviceCache = {
    orm: {},
    rpc: {},
};
...
const SnippetOptionWidget = publicWidget.Widget.extend({
    ...
    _select: async function (previewMode, widget) {
        ...
        if (previewMode === true) {
            this.options.wysiwyg.edenEditor.automaticStepUnactive('preview_option');
        }
        ...
        if (previewMode === 'reset' || previewMode === false) {
            this.options.wysiwyg.edenEditor.automaticStepActive('preview_option');
        }
    ...
    start: function () {
        ...
        this.$handles.on('mousedown', function (ev) {
            ...
            if (rowEl.classList.contains("o_grid_mode") && !isMobile) {
                self.options.wysiwyg.edenEditor.observerUnactive('displayBackgroundGrid');
                backgroundGridEl = gridUtils._addBackgroundGrid(rowEl, 0);
                gridUtils._setElementToMaxZindex(backgroundGridEl, rowEl);
                self.options.wysiwyg.edenEditor.observerActive('displayBackgroundGrid');
            }
            ...
            self.options.wysiwyg.edenEditor.automaticStepUnactive('resizing');
            ...
            const iframeWindowMouseUp = function (ev) {
                ...
                self.options.wysiwyg.edenEditor.automaticStepActive('resizing');
                ...
                setTimeout(function () {
                    self.options.wysiwyg.edenEditor.historyStep();
...
registry.layout_column = SnippetOptionWidget.extend(ColumnLayoutMixin, {
    ...
    async addElement(previewMode, widgetValue, params) {
        ...
        if (elementType === 'image') {
            ...
            if (!isImageSaved) {
                ...
                // media dialog closed.
                this.options.wysiwyg.edenEditor.historyRevertCurrentStep();
                return;
            }
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        if (["row-gap", "column-gap"].includes(params.cssProperty) && !isMobileView) {
            ...
            this.options.wysiwyg.edenEditor.observerUnactive("addGridPreview");
            ...
            this.options.wysiwyg.edenEditor.observerActive("addGridPreview");
     ...
     */
    _removeGridPreview() {
        this.options.wysiwyg.edenEditor.observerUnactive("removeGridPreview");
        ...
        this.options.wysiwyg.edenEditor.observerActive("removeGridPreview");
    },
    /**
     * @returns {boolean}
     */
...
registry.GridColumns = SnippetOptionWidget.extend({
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        if (["--grid-item-padding-y", "--grid-item-padding-x"].includes(params.cssProperty)) {
            ...
            this.options.wysiwyg.edenEditor.observerUnactive("addPaddingPreview");
            this.$target[0].classList.add("o_we_padding_highlight");
            this.options.wysiwyg.edenEditor.observerActive("addPaddingPreview");
    ...
    _removePaddingPreview() {
        this.options.wysiwyg.edenEditor.observerUnactive("removePaddingPreview");
        ...
        this.options.wysiwyg.edenEditor.observerActive("removePaddingPreview");
    },
});
...
registry.ReplaceMedia = SnippetOptionWidget.extend({
     ...
     */
    onFocus() {
        this.options.wysiwyg.edenEditor.addEventListener('activate_image_link_tool', this._activateLinkTool);
        this.options.wysiwyg.edenEditor.addEventListener('deactivate_image_link_tool', this._deactivateLinkTool);
     ...
     */
    _clearListeners() {
        this.options.wysiwyg.edenEditor.removeEventListener('activate_image_link_tool', this._activateLinkTool);
        this.options.wysiwyg.edenEditor.removeEventListener('deactivate_image_link_tool', this._deactivateLinkTool);
    },
    /**
     * @private
     */
...
￼    */
    ...
    async _writeShape(svgText) {
        ...
        if (hasHoverEffect) {
            ...
            clonedImgEl = img.cloneNode(true);
            this.options.wysiwyg.edenEditor.observerUnactive("addClonedImgForHoverEffectPreview");
            img.classList.add("d-none");
            img.insertAdjacentElement("afterend", clonedImgEl);
            this.options.wysiwyg.edenEditor.observerActive("addClonedImgForHoverEffectPreview");
        }
        const loadedImg = await loadImage(dataURL, img);
        if (hasHoverEffect) {
            this.options.wysiwyg.edenEditor.observerUnactive("removeClonedImgForHoverEffectPreview");
            clonedImgEl.remove();
            img.classList.remove("d-none");
            this.options.wysiwyg.edenEditor.observerActive("removeClonedImgForHoverEffectPreview");
        }
    ...
    _computeMaxDisplayWidth() {
        ...
        // width since we only use col-lg-* in Eden).
...
registry.BackgroundImage = SnippetOptionWidget.extend({
    ...
    _setBackground(backgroundURL) {
        ...
        this.options.wysiwyg.edenEditor.editable.focus();
    },
});
...
registry.ColoredLevelBackground = registry.BackgroundToggler.extend({
    ...
    _markColorLevel: function () {
        this.options.wysiwyg.edenEditor.observerUnactive('_markColorLevel');
        this.$target.addClass('o_colored_level');
        this.options.wysiwyg.edenEditor.observerActive('_markColorLevel');
    },
});
