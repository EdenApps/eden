/** @eden-module **/
import { setSelection } from "@web_editor/js/editor/eden-editor/src/utils/utils";
...
QUnit.module(
    ...
    function () {
        ...
        QUnit.test("typing valid URL + space should convert to link", async function (assert) {
                ...
                const node = editable.querySelector("p");
                setSelection(node, 0);
                inputText(".eden-editor-editable p", "http://google.co.in");
                insertText(editor, " ");
                editor.clean();
                assert.strictEqual(
                    editable.innerHTML,
        ...
        QUnit.test("typing invalid URL + space should not convert to link", async function (assert) {
            ...
            const node = editable.querySelector("p");
            setSelection(node, 0);
            inputText(".eden-editor-editable p", "www.eden");
            insertText(editor, " ");
            editor.clean();
            assert.strictEqual(editable.innerHTML, "<p>www.eden </p>");
        });
