...
import { press, queryAll, queryAllTexts, queryOne } from "@eden/hoot-dom";
import { Deferred, advanceTime, animationFrame, runAllTimers } from "@eden/hoot-mock";
import {
    contains,
    getService,
    mountWithCleanup,
...
test("checks that href is correctly used", async () => {
    ...
    expect(".o_command_palette .o_command:eq(0) a").toHaveAttribute("href", "https://www.edencloud.us");
    // Check that we get url when doing ctrl+enter on a command having a link inside it
    await press("control+enter");
    await animationFrame();
    expect.verifySteps(["https://www.edencloud.us"]);
