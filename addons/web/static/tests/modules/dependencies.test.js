...
function invalidImportsFrom(folder, allowedFolders) {
    ...
    const modulesToCheck = Array.from(eden.loader.modules.keys()).filter((module) =>
        module.startsWith(`@web/${folder}/`)
    );
    const invalidDeps = {};
    for (const module of modulesToCheck) {
        const invalid = eden.loader.factories.get(module).deps.filter((dep) => {
            // owl and @web/session are allowed everywhere
            if (dep === "@eden/owl" || dep === "@web/session") {
                return false;
            }
