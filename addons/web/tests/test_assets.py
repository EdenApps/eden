...
class TestAssetsGenerateTimeCommon(eden.tests.TransactionCase):
...
@eden.tests.tagged('post_install', '-at_install', 'assets_bundle')
...
@eden.tests.tagged('post_install', '-at_install', '-standard', 'test_assets')
class TestPregenerateTime(HttpCase):
    ...
    def test_logs_pregenerate_time(self):
        ...
        with self.profile(collectors=['sql', eden.tools.profiler.PeriodicCollector(interval=0.01)], disable_gc=True):
            self.env['ir.qweb']._pregenerate_assets_bundles()
        duration = time.time() - start
        _logger.info('All bundle checked in %.2fs', duration)

@eden.tests.tagged('post_install', '-at_install', '-standard', 'assets_bundle')
...
class TestLoad(HttpCase):
    def test_assets_already_exists(self):
        ...
        _save_attachment = eden.addons.base.models.assetsbundle.AssetsBundle.save_attachment
        ...
        with patch('eden.addons.base.models.assetsbundle.AssetsBundle.save_attachment', save_attachment):
            self.url_open('/eden').raise_for_status()
