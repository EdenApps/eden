...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        # simple event values to create a Eden event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Eden events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to Eden events
        # -----------------------------------------------------------------------------------------

        self.expected_eden_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_eden_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_eden_recurrency_events_from_outlook = [
            {
    ...
    def sync_eden_recurrences_with_outlook_feature(self):
        """
    ...
    def create_events_for_tests(self):
        ...
        # Currently, it is forbidden to create recurrences in Eden. A trick for deactivating the checking
    ...
    def assert_eden_event(self, eden_event, expected_values):
        """
        Assert that an Eden event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        eden_event_values = eden_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(eden_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, eden_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_eden_recurrence(self, eden_recurrence, expected_values):
        """
        Assert that an Eden recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        eden_recurrence_values = eden_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, eden_recurrence_values.get(k), msg=f"'{k}' mismatch")
