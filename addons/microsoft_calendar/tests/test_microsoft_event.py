...
from eden.addons.microsoft_calendar.tests.common import TestCommon, patch_api
...
class TestMicrosoftEvent(TestCommon):
    ...
    def test_already_mapped_events(self):
        ...
        events = MicrosoftEvent([{
            "type": "singleInstance",
            "_eden_id": self.simple_event.id,
            "iCalUId": event_uid,
            "id": event_id,
        }])

        # act
        mapped = events._load_eden_ids_from_db(self.env)

        # assert
        self.assertEqual(len(mapped._events), 1)
        self.assertEqual(mapped._events[event_id]["_eden_id"], self.simple_event.id)
    ...
    def test_map_an_event_using_instance_id(self):
        """
        Here, the Eden event has an uid but the Outlook event has not.
        """
        # arrange
        event_id = self.simple_event.microsoft_id
        events = MicrosoftEvent([{
    ...
    def test_map_an_event_without_uid_using_instance_id(self):
        """
        Here, the Eden event has no uid but the Outlook event has one.
        """
    ...
    def test_map_an_event_without_uid_using_instance_id_2(self):
        """
        Here, both Eden event and Outlook event have no uid.
        """
    ...
    def test_map_a_recurrence_using_global_id(self):
        ...
        # assert
        self.assertEqual(len(mapped._events), 1)
        self.assertEqual(mapped._events[rec_id]["_eden_id"], self.recurrence.id)
    ...
    def test_try_to_map_mixed_of_single_events_and_recurrences(self):
        ...
        with self.assertRaises(TypeError):
            events._load_eden_ids_from_db(self.env)
    ...
    def test_match_event_only(self):
        ...
        # act
        matched = events.match_with_eden_events(self.env)

        # assert
        self.assertEqual(len(matched._events), 1)
        self.assertEqual(matched._events[event_id]["_eden_id"], self.simple_event.id)
    ...
    def test_match_recurrence_only(self):
        ...
        # assert
        self.assertEqual(len(matched._events), 1)
        self.assertEqual(matched._events[rec_id]["_eden_id"], self.recurrence.id)
    ...
    def test_performance_check(self):
        ...
        # Other tests take care of how we update eden records with the api result.
        ...
        mapped = events._load_eden_ids_from_db(self.env)
        self.assertFalse(mapped, "No eden record should correspond to the microsoft values")
        ...
        mapped = recurrences._load_eden_ids_from_db(self.env)
        self.assertFalse(mapped, "No eden record should correspond to the microsoft values")
