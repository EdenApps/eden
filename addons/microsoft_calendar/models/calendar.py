...
class Meeting(models.Model):
    ...
    def create(self, vals_list):
        ...
        # Forbid recurrence creation in Eden, suggest its creation in Outlook due to the spam limitation.
            ...
            if not sender_sync_status and current_sync_status:
                raise ValidationError(
                    _("For having a different organizer in your event, it is necessary that "
                      "the organizer have its Eden Calendar synced with Outlook Calendar."))
    ...
    def _forbid_recurrence_update(self):
        ...
        if any(not record.ms_universal_event_id for record in self):
            # If any event is not synced, suggest deleting it in Eden and recreating it in Outlook.
            error_msg = _(
                "Due to an Outlook Calendar limitation, recurrence updates must be done directly in Outlook Calendar.\n"
                "If this recurrence is not shown in Outlook Calendar, you must delete it in Eden Calendar and recreate it in Outlook Calendar.")
    ...
    def write(self, values):
        ...
        # Forbid recurrence updates through Eden and suggest user to update it in Outlook.
    ...
    def _get_microsoft_sync_domain(self):
        ...
        # Define 'custom_lower_bound_range' param for limiting old events updates in Eden and avoid spam on Microsoft.
    ...
    @api.model
    def _microsoft_to_eden_values(self, microsoft_event, default_reminders=(), default_values=None, with_ids=False):
        ...
        commands_attendee, commands_partner = self._eden_attendee_commands_m(microsoft_event)
        ...
        alarm_commands = self._eden_reminders_commands_m(microsoft_event)
        if alarm_commands:
    ...
    @api.model
    def _microsoft_to_eden_recurrence_values(self, microsoft_event, default_values=None):
        timeZone_start = pytz.timezone(microsoft_event.start.get('timeZone'))
        timeZone_stop = pytz.timezone(microsoft_event.end.get('timeZone'))
        start = parse(microsoft_event.start.get('dateTime')).astimezone(timeZone_start).replace(tzinfo=None)
        if microsoft_event.isAllDay:
    ...
    @api.model
    def _eden_attendee_commands_m(self, microsoft_event):
        commands_attendee = []
        commands_partner = []
        ...
        if microsoft_event.match_with_eden_events(self.env):
            existing_attendees = self.env['calendar.attendee'].search([
                ('event_id', '=', microsoft_event.eden_id(self.env)),
                ('email', 'in', emails)])
        ...
        for eden_attendee in attendees_by_emails.values():
            # Remove old attendees
            if eden_attendee.email not in emails:
                commands_attendee += [(2, eden_attendee.id)]
                commands_partner += [(3, eden_attendee.partner_id.id)]
        return commands_attendee, commands_partner

    @api.model
    def _eden_reminders_commands_m(self, microsoft_event):
        reminders_commands = []
        if microsoft_event.isReminderOn:
            event_id = self.browse(microsoft_event.eden_id(self.env))
    ...
    def _ensure_attendees_have_email(self):
        ...
        if invalid_event_ids:
            ...
            raise ValidationError(_("For a correct synchronization between Eden and Outlook Calendar, "
    ...
    def _cancel_microsoft(self):
        ...
        There are 2 cases:
          1) the organizer is an Eden user: he's the only one able to delete the Eden event. Attendees can just decline.
          2) the organizer is NOT an Eden user: any attendee should remove the Eden event.
        """
        user = self.env.user
