...
from eden.addons.microsoft_account.models.microsoft_service import TIMEOUT
...
# This ensures changes are sent to Microsoft only if they really happened in the Eden database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Microsoft if the first creation crashed in Eden.
...
class MicrosoftSync(models.AbstractModel):
    ...
    def _sync_recurrence_microsoft2eden(self, microsoft_events, new_events=None):
        ...
        for recurrent_master in recurrent_masters:
            new_calendar_recurrence = dict(
                self.env['calendar.recurrence']._microsoft_to_eden_values(recurrent_master, default_values, with_ids=True),
                need_sync_m=False
            )
            ...
            base_values = dict(
                self.env['calendar.event']._microsoft_to_eden_values(recurrent_master, default_values, with_ids=True),
                need_sync_m=False
            )
            to_create_values = []
            ...
            for recurrent_event in to_create:
                if recurrent_event.type == 'occurrence':
                    value = self.env['calendar.event']._microsoft_to_eden_recurrence_values(recurrent_event, base_values)
                else:
                    value = self.env['calendar.event']._microsoft_to_eden_values(recurrent_event, default_values)
            ...
            new_recurrence_eden = self.env['calendar.recurrence'].with_context(dont_notify=True).create(new_calendar_recurrence)
            new_recurrence_eden.base_event_id = new_recurrence_eden.calendar_event_ids[0] if new_recurrence_eden.calendar_event_ids else False
            new_recurrence |= new_recurrence_eden
        ...
        for recurrent_master_id in ms_recurrence_ids:
            ...
            for recurrent_event in to_update:
                if recurrent_event.type == 'occurrence':
                    value = self.env['calendar.event']._microsoft_to_eden_recurrence_values(
    ...
    def _update_microsoft_recurrence(self, recurrence, events):
        """
        Update Eden events from Outlook recurrence and events.
        """
        ...
        for e in events_to_update:
            if e.type == "exception":
                event_values = self.env['calendar.event']._microsoft_to_eden_values(e)
            elif e.type == "occurrence":
                event_values = self.env['calendar.event']._microsoft_to_eden_recurrence_values(e)
            else:
                event_values = None
            ...
            if event_values:
                ...
                eden_event = self.env['calendar.event'].browse(e.eden_id(self.env)).exists().with_context(
                    no_mail_to_attendees=True, mail_create_nolog=True
                )
                eden_event.with_context(dont_notify=True).write(dict(event_values, need_sync_m=False))
                update_events |= eden_event
    ...
    @api.model
    def _sync_microsoft2eden(self, microsoft_events: MicrosoftEvent):
        """
        Synchronize Microsoft recurrences in Eden.
        Creates new recurrences, updates existing ones.
        :return: synchronized eden
        """
        existing = microsoft_events.match_with_eden_events(self.env)
        ...
        eden_values = [
            dict(self._microsoft_to_eden_values(e, with_ids=True), need_sync_m=False)
            for e in (new - new_recurrence)
        ]
        synced_events = self.with_context(dont_notify=True)._create_from_microsoft(new, eden_values)
        synced_recurrences, updated_events = self._sync_recurrence_microsoft2eden(existing, new_recurrence)
        ...
        # Get sync lower bound days range for checking if old events must be updated in Eden.
        ...
        for mevent in (existing - cancelled).filter(lambda e: e.lastModifiedDateTime):
            # Last updated wins.
            # This could be dangerous if microsoft server time and eden server time are different
            if mevent.is_recurrence():
                eden_event = self.env['calendar.recurrence'].browse(mevent.eden_id(self.env)).exists()
            else:
                eden_event = self.browse(mevent.eden_id(self.env)).exists()

            if eden_event:
                eden_event_updated_time = pytz.utc.localize(eden_event.write_date)
                ...
                if lower_bound_day_range:
                    update_time_diff = ms_event_updated_time - eden_event_updated_time
                    old_event_update_condition = eden_event._check_old_event_update_required(int(lower_bound_day_range), update_time_diff)

                if ms_event_updated_time >= eden_event_updated_time and old_event_update_condition:
                    vals = dict(eden_event._microsoft_to_eden_values(mevent), need_sync_m=False)
                    eden_event.with_context(dont_notify=True)._write_from_microsoft(mevent, vals)

                    if eden_event._name == 'calendar.recurrence':
                        update_events = eden_event._update_microsoft_recurrence(mevent, microsoft_events)
                        synced_recurrences |= eden_event
                        synced_events |= update_events
                    else:
                        synced_events |= eden_event

        return synced_events, synced_recurrences

    def _check_old_event_update_required(self, lower_bound_day_range, update_time_diff):
        """
        Checks if an old event in Eden should be updated locally. This verification is necessary because
        sometimes events in Eden have the same state in Microsoft and even so they trigger updates locally
    ...
    def _microsoft_delete(self, user_id, event_id, timeout=TIMEOUT):
        """
        Once the event has been really removed from the Eden database, remove it from the Outlook calendar.
    ...
    def _microsoft_patch(self, user_id, event_id, values, timeout=TIMEOUT):
        """
        Once the event has been really modified in the Eden database, modify it in the Outlook calendar.
    ...
    def _microsoft_insert(self, values, timeout=TIMEOUT):
        """
        Once the event has been really added in the Eden database, add it in the Outlook calendar.
    ...
    def _microsoft_attendee_answer(self, answer, params, timeout=TIMEOUT):
        ...
        with microsoft_calendar_token(self.env.user.sudo()) as token:
            if token:
                ...
                # former differs for each attendee. This info is required for sending the event answer and Eden currently
    ...
    def _get_microsoft_records_to_sync(self, full_sync=False):
        """
        Return records that should be synced from Eden to Microsoft
    ...
    @api.model
    def _microsoft_to_eden_values(
        self, microsoft_event: MicrosoftEvent, default_reminders=(), default_values=None, with_ids=False
    ):
        """
        Implements this method to return a dict of Eden values corresponding
        to the Microsoft event given as parameter
        :return: dict of Eden formatted values
        """
